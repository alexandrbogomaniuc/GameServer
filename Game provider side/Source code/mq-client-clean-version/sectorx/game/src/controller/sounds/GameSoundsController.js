...
class GameSoundsController extends SoundsController
{
	...
	_playBgMusic(aSoundName_str, aOptAutoplay_bl = true, aOptSmoothly_bl = false, aOptStopPreviousBgSound_bl = true)
	{
		...
		if (this._fMusicBg_uscc && aOptAutoplay_bl)
		{
			...
			if (aSoundName_str != "mq_mus_game_bg_boss")
			{
	...
	_onSomeBigWinPresentationStarted(aEvent_evnt)
	{
		...
		this._fBigWinSound_ussc && this._fBigWinSound_ussc.once(SimpleSoundController.i_EVENT_SOUND_PLAYING_COMPLETED, this._clearBigWinSound, this);
	...
	_clearBigWinSound(aEvent_obj)
	{
		...
		let lMusicSoundController_mssc = aEvent_obj.target;
