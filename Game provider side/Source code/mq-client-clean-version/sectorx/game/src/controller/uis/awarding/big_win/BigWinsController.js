...
class BigWinsController extends SimpleUIController
{
	...
	__initControlLevel()
	{
		...
		this._gameScreen.bossModeController.on(BossModeController.EVENT_ON_TIME_TO_DEFEATED_CAPTION, this._onBossModeTimeToShowDefeatedCaption, this);
	...
	_onBulletTargetTime(aEvent_obj)
	{
		...
		{
			l_enm.once(Enemy.EVENT_ON_DEATH_COIN_AWARD, this._onEnemyBigWinTime.bind(this, result, lIsBoss_bl), this);
		}
		else
		{
	...
	_checkIfBigWinAnimationNeeded(data)
	{
		...
		for (let affectedEnemy of data.affectedEnemies)
		{
			...
			{
				if(affectedEnemy.data.chMult > 0 && data.bombExplode)
				{
					return false;
				}
				...
				if (innerData.hitResultBySeats && innerData.hitResultBySeats[currentSeat])
				{
					...
					{
						if (data.enemy.typeId === ENEMY_TYPES.GOLD_CAPSULE && +award.value > 0)
						{
		...
		{
			return {totalWin: lTotalWin_num, shotStake: lShotStake_num, isMoneyWheel: (lMoneyWheelsAmount_num > 0), rid: data.rid};
		}
		return false;
	}

	_onEnemyBigWinTime(aCheckBigWinResult_obj, aIsBoss_bl=false)
	{
		...
		let l_bwc = this._generateBigWinController(aCheckBigWinResult_obj.totalWin, aCheckBigWinResult_obj.shotStake, aIsBoss_bl);
		this._startBigWinAnimation(l_bwc);
	}

	_generateBigWinController(aTotalWin_num, aShotStake_num, aOptIsBoss_bl=false)
	{
