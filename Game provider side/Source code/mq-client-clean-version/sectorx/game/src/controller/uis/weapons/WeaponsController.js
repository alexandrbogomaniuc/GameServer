...
class WeaponsController extends SimpleController
{
		...
		switch(data.class)
		{
			...
			case SERVER_MESSAGES.BALANCE_UPDATED:
				...
				if (data.rid !== -1)
				{
					...
					let lNonRicochetBullets_num = APP.gameScreen.fireController.getMasterNonRicochetBulletsAmount();
					...
					{
						if ((clientResultAmmo + lRicochetBullets_num*this._fPlayerInfo_pi.betLevel + lNonRicochetBullets_num*this._fPlayerInfo_pi.betLevel) != serverAmmo)
						{
							...
							console.error("*** WRONG AMMO*** clientResultAmmo:", clientResultAmmo, "; serverAmmo:", serverAmmo, "; clientAmmo:", clientAmmo, "; clientPendingAmmo:", clientPendingAmmo, 
												"; ricochetBullets:", lRicochetBullets_num, "; nonRicochetBullets:", lNonRicochetBullets_num, "; bet level:", this._fPlayerInfo_pi.betLevel);
						}
					}
				}
				//...debug: compare ammo amount on client/server
	...
	_onRevertAmmoBack(aEvent_obj)
	{
		...
		if (!aEvent_obj.isPaidSpecialShot)
		{
