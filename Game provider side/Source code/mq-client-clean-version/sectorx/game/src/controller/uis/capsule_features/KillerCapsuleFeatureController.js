...
class KillerCapsuleFeatureController extends SimpleUIController
{
	...
	static get EVENT_ON_ALL_ANIMATIONS_COMPLETED()					{ return KillerCapsuleFeatureView.EVENT_ON_ALL_ANIMATIONS_COMPLETED; }
	...
	get isAnimationPlaying()
	{
		...
		this._fViewCapsuleFeature_kcfv_arr && this._fViewCapsuleFeature_kcfv_arr.forEach(
			function(el, index) 
			{
				if (el.isAnimationPlaying)
				{
	...
	_startAnimation(aStartPostion_obj)
	{
		...
		lView_kcfv.on(KillerCapsuleFeatureView.EVENT_ON_ALL_ANIMATIONS_COMPLETED, this.emit, this);
	...
	_onRoomClear()
	{
		this._fViewCapsuleFeature_kcfv_arr.forEach(element => {
			element.interrupt();
		});
	}
