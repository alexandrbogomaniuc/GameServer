...
class FireController extends SimpleUIController
{
	...
	getMasterNonRicochetBulletsAmount()
	{
		let l_num = 0;
		for (let bullet of this.bullets)
		{
	...
	chooseFireType(e)
	{
		console.log("auto fire problem  chooseFireType" );
	...
	fireImmediately()
	{
		console.log("auto fire problem  fireImmediately" );
		this.fire({ 
			data: {
				global:{
	...
	get _isFireDenied()
	{
		return	this._fGameStateInfo_gsi.gameState != ROUND_STATE.PLAY
				...
				|| APP.isBattlegroundGame && this._gameField.battlegroundFinalCountingController.info.isFinalCountingFireDenied
	...
	{
		console.log(" auto fire problem fire");
		//debug info for QA...
		...
		let lIsFreeShot_bl = this._isFrbMode;
		...
		if (this._isFireDenied)
		{
			...
			else if (!this._isEnoughMoneyForOneShot() && !this._gameField.roundResultActive)
			{
				...
				if(
					...
					&&
					(
						this._gameField.roundResultActivationInProgress ||
				...
				if (APP.playerController.info.unpresentedWin > 0) //wait until the win is presented
				{
					return;
				}

				this.emit(
					FireController.EVENT_NOT_ENOUGH_MONEY_DIALOG_REQUIRED,
					{
		...
		if ( !lIsFreeShot_bl && !lIsBattlegroundDefaultShot_bl && lWeaponsInfo_wsi.ammo < lShotAmmoAmount_int)
		{
			...
			if (!this._gameField.isAmmoBuyingInProgress)
			{
				...
				if (lPlayerInfo_pi.balance < lShotCost_num && !lPlayerInfo_pi.unpresentedWin)
				{
					...
					else if (lRicochetBullets_num || lNonRicochetBullets_num)
					{
						return; // No actions needed if we don't have money, but have bullets on screen
					}
					else
					{
						this.emit(FireController.EVENT_NOT_ENOUGH_MONEY_DIALOG_REQUIRED, {
																						hasUnrespondedShots: APP.webSocketInteractionController.hasUnrespondedShots, 
																						hasDelayedShots: APP.webSocketInteractionController.hasDelayedShots,
																						hasUnparsedShotResponse: APP.webSocketInteractionController.isShotRequestParseInProgress,
																					}
		...
		if (!isRicochet)
		{
			...
			bulletProps.weapon = lCurrentWeaponId_num;
	...
	*/
	...
	_onEnemyClick(aEnemy_e)
	{
		...
		else if (this._fAutoTargetingSwitcherInfo_atsi.isOn && this._fFireSettingsInfo_fsi.lockOnTarget)
		{
