...
class BossModeController extends SimpleUIController
{
	...
	static get EVENT_ON_TIME_TO_DEFEATED_CAPTION()			{return BossModeView.EVENT_ON_TIME_TO_DEFEATED_CAPTION;}
	...
	_onAppearingPresentationStarted(event)
	{
	...
	_onTimeToDefeatedCaption()
	{
		...
		this.emit(BossModeController.EVENT_ON_TIME_TO_DEFEATED_CAPTION);
	}
	...
	_onUpdatePlayerWinCaption(event)
	{
		let lView_bmv = this.view;
		lView_bmv.addToContainerIfRequired(this._gameScreen.gameFieldController.bossModeAppearingContainerInfo);
		lView_bmv.updatePlayerWinCaption(event.playerName);
	}
