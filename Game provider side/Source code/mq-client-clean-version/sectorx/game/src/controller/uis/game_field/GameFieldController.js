...
export const Z_INDEXES = {
	...
	TRANSITION_ROUND_END: 						110002, /*this*/
	...
	SCOREBOARD: 								20130, /*this.view.screen*/
...
class GameFieldController extends SimpleUIController
{
	...
	onFireSettingsChanged()
	{
		this._onFireSettingsChanged();
	}
	...
	get transitionRouneEndContainerInfo()
	{
		return { container: this.view, zIndex: Z_INDEXES.TRANSITION_ROUND_END };
	}
	...
	{
		return { conatiner: this.view.screen, zIndex: Z_INDEXES.SCOREBOARD };
	}
	...
	startGamePlay()
	{
		if (!this.view.screen)
		{
			...
			APP.gameScreen._gameplayDialogController.init();
	...
	onRoundResultScreenActivated()
	{
		this._onRoundResultScreenActivated();
	}
	...
	changeState(state)
	{
		...
		this.emit(GameFieldController.EVENT_ON_ROUND_STATE_CHANGED, { newState: state });
	}
	...
	_resetCoPlayersWeaponsIfRequired()
	{
		let lPlayerInfo_pi = APP.playerController.info;
		...
		if (
			...
			&& lGameStateInfo_gsi.isWaitState
	...
	{
		APP.gameScreen.transitionViewController.startListenForRoundEnd();
		this._stopListenForRoundEnd();
		...
		let lAwardingController = APP.currentWindow.awardingController;
		if (lAwardingController && lAwardingController.isAnyAwardingInProgress)
		{
			++this._fRoundResultAnimsCount_num;
			lAwardingController.once(AwardingController.EVENT_ON_ALL_ANIMATIONS_COMPLETED, this._onAwardsAnimationsCompleted, this);

			this._fRoundEndListenHandlers_arr.push({
				obj: lAwardingController,
				event: AwardingController.EVENT_ON_ALL_ANIMATIONS_COMPLETED,
				handler: this._onAwardsAnimationsCompleted
			});
		}

		let lBigWinsController = APP.currentWindow.bigWinsController;
		if (lBigWinsController && lBigWinsController.isAnyBigWinInProgress)
		{
			++this._fRoundResultAnimsCount_num;
			lBigWinsController.once(BigWinsController.EVENT_ON_ALL_ANIMATIONS_COMPLETED, this._onBigWinsAnimationsCompleted, this);
			this._fRoundEndListenHandlers_arr.push({
				obj: lBigWinsController,
				event: BigWinsController.EVENT_ON_ALL_ANIMATIONS_COMPLETED,
				handler: this._onBigWinsAnimationsCompleted
			});

			lBigWinsController.once(BigWinsController.EVENT_ON_ANIMATION_INTERRUPTED, this._onBigWinsAnimationsInterrupted, this);
			this._fRoundEndListenHandlers_arr.push({
				obj: lBigWinsController,
		...
		const lMoneyWheelController = this.moneyWheelController;
		if ((lMoneyWheelController && lMoneyWheelController.isAnimInProgress) || lAwardingController.isAnyMoneyWheelAwardExpected)
		{
			++this._fRoundResultAnimsCount_num;
			lMoneyWheelController.once(MoneyWheelController.EVENT_ON_ALL_ANIMATIONS_COMPLETED, this._onMoneyWheelsAnimationsCompleted, this);
			this._fRoundEndListenHandlers_arr.push({
				obj: lMoneyWheelController,
				event: MoneyWheelController.EVENT_ON_ALL_ANIMATIONS_COMPLETED,
				handler: this._onMoneyWheelsAnimationsCompleted,
			});
		}

		const lBombController = this.bombController;
		if (lBombController && lBombController.isAnimInProgress)
		{
			++this._fRoundResultAnimsCount_num;
			lBombController.once(BombController.EVENT_ON_ALL_ANIMATIONS_COMPLETED, this._onBombsAnimationsCompleted, this);
			this._fRoundEndListenHandlers_arr.push({
				obj: lBombController,
		...
		let lFreezeController = APP.gameScreen.freezeCapsuleFeatureController;
		if (lFreezeController.isFreezingInProgress)
		{
			++this._fRoundResultAnimsCount_num;
			APP.gameScreen.once(FreezeCapsuleFeatureController.EVENT_ON_UNFREEZE_AFFECTED_ENEMIES, this._onFreezeAnimationsCompleted, this);

			this._fRoundEndListenHandlers_arr.push({
				obj: lFreezeController,
				event: FreezeCapsuleFeatureController.EVENT_ON_UNFREEZE_AFFECTED_ENEMIES,
				handler: this._onFreezeAnimationsCompleted
			});
		}

		const lBulletCapsuleFeatureController = APP.gameScreen.bulletCapsuleFeatureController;
		if ((lBulletCapsuleFeatureController && lBulletCapsuleFeatureController.isAnimationPlaying))
		{
			++this._fRoundResultAnimsCount_num;
			lBulletCapsuleFeatureController.once(BulletCapsuleFeatureController.EVENT_ON_ALL_ANIMATIONS_COMPLETED, this._onBulletCapsuleFeatureAnimationsCompleted, this);
			this._fRoundEndListenHandlers_arr.push({
				obj: lBulletCapsuleFeatureController,
				event: BulletCapsuleFeatureController.EVENT_ON_ALL_ANIMATIONS_COMPLETED,
				handler: this._onBulletCapsuleFeatureAnimationsCompleted,
			});
		}

		const lLaserCapsuleFeatureController = APP.gameScreen.laserCapsuleFeatureController;
		if ((lLaserCapsuleFeatureController && lLaserCapsuleFeatureController.isAnimationPlaying))
		{
			++this._fRoundResultAnimsCount_num;
			lLaserCapsuleFeatureController.once(LaserCapsuleFeatureController.EVENT_ON_ALL_ANIMATIONS_COMPLETED, this._onLaserCapsuleFeatureAnimationsCompleted, this);
			this._fRoundEndListenHandlers_arr.push({
				obj: lLaserCapsuleFeatureController,
				event: LaserCapsuleFeatureController.EVENT_ON_ALL_ANIMATIONS_COMPLETED,
				handler: this._onLaserCapsuleFeatureAnimationsCompleted,
			});
		}

		const lLightningCapsuleFeatureController = APP.gameScreen.lightningCapsuleFeatureController;
		if ((lLightningCapsuleFeatureController && lLightningCapsuleFeatureController.isAnimationPlaying))
		{
			++this._fRoundResultAnimsCount_num;
			lLightningCapsuleFeatureController.once(LightningCapsuleFeatureController.EVENT_ON_ALL_ANIMATIONS_COMPLETED, this._onLightningCapsuleFeatureAnimationsCompleted, this);
			this._fRoundEndListenHandlers_arr.push({
				obj: lLightningCapsuleFeatureController,
				event: LightningCapsuleFeatureController.EVENT_ON_ALL_ANIMATIONS_COMPLETED,
				handler: this._onLightningCapsuleFeatureAnimationsCompleted,
			});
		}

		const lKillerCapsuleFeatureController = APP.gameScreen.killerCapsuleFeatureController;
		if ((lKillerCapsuleFeatureController && lKillerCapsuleFeatureController.isAnimationPlaying))
		{
			++this._fRoundResultAnimsCount_num;
			lKillerCapsuleFeatureController.once(KillerCapsuleFeatureController.EVENT_ON_ALL_ANIMATIONS_COMPLETED, this._onKillerCapsuleFeatureAnimationsCompleted, this);
			this._fRoundEndListenHandlers_arr.push({
				obj: lKillerCapsuleFeatureController,
				event: KillerCapsuleFeatureController.EVENT_ON_ALL_ANIMATIONS_COMPLETED,
				handler: this._onKillerCapsuleFeatureAnimationsCompleted,
			});
		}
	...
	_onBulletCapsuleFeatureAnimationsCompleted()
	{
		this.onNextAnimationEnd();
	}

	_onLaserCapsuleFeatureAnimationsCompleted()
	{
		this.onNextAnimationEnd();
	}

	_onLightningCapsuleFeatureAnimationsCompleted()
	{
		this.onNextAnimationEnd();
	}

	_onKillerCapsuleFeatureAnimationsCompleted()
	{
		this.onNextAnimationEnd();
	}
	...
	changeWeapon(weaponId, aIgnoreRoundResult_bln = false, aIsSkipAnimation_bl = false, aIsNewAwardedLevelUp_bl = false)
	{
		...
		if (lCurrentWeaponId_int === weaponId && !this._fWeaponSwitchTry_bln)
		{
			if (this.spot)
			{
				if ((!lWeaponsInfo_wsi.isFreeWeaponsQueueActivated))
				{
	...
	_onFireSettingsChanged()
	{
		...
		{
			APP.gameScreen.fireController.i_resetTargetIfRequired(true);
		}
	}
	...
	showFire(data, callback)
	{
		...
		if (!lIsRicochetBulletResultData_bl)
		{
			APP.gameScreen.fireController.showPlayersWeaponEffect(data, angle, points[1]);
		}
	...
	showPlayersWeaponSwitched(data)
	{
		...
		for (var i = 0; i < this.players.length; i++)
		{
			if (this.players[i].seatId == data.seatId && data.rid == -1)
			{
				...
				{
					let lSpotCurrentDefaultWeaponId_int = APP.playerController.info.getTurretSkinId(this.players[i].betLevel);
	...
	showMissAnimation(endPos, angle, data)
	{
		...
		if (enemy)
		{
			if (data.killedMiss)
			{
				...
				{
					this.emit(GameFieldController.EVENT_ON_UPDATE_PLAYER_WIN_CAPTION, { playerName: data.playerName })
				}
	...
	_onShowDalayedLaserCapsuleAward()
	{
		this._fDelayedLaserExploasionHits_obj_arr && this._fDelayedLaserExploasionHits_obj_arr.forEach((lDelayedLaserExploasionHitData) =>
		{
			...
			if (lDelayedLaserExploasionHitData.class == 'Hit') this.showHitAnimation(null, angle, lDelayedLaserExploasionHitData);
		});
		this._fDelayedLaserExploasionHits_obj_arr = [];
	}
	...
	*/
	...
	createEnemy(aEnemy_obj)
	{
		...
		this.emit(GameFieldController.EVENT_ON_NEW_ENEMY_CREATED, { enemyId: aEnemy_obj.id });
