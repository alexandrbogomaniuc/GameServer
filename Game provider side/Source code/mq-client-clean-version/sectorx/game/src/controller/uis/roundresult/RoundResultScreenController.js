...
import ScoreboardItem from '../../../view/uis/scoreboard/ScoreboardItem';
...
class RoundResultScreenController extends SimpleUIController
{
	...
	static get EVENT_ON_BATTLEGROUND_ROUND_RESULT_TIMER_EXPIRED() { return "EVENT_ON_BATTLEGROUND_ROUND_RESULT_TIMER_EXPIRED"; }
	...
	__initControlLevel()
	{
		...
		this._fGameScreen_gs.on(GameScreen.EVENT_BATTLEGROUND_TIME_TO_START_UPDATED, this._onBattlegroundTimeToStartUpdated, this, true);
		...
		wsInteractionController.on(GameWebSocketInteractionController.EVENT_ON_SERVER_ROUND_RESULT_MESSAGE, this._onRoundResultResponse, this);
	}
	...
	_onRoundResultResponse(aData_obj)
	{
		...
		*/
		...
		let lData_obj = aData_obj.messageData;
		let lBattlegroundInfo_obj_arr = lData_obj.battlegroundRoundResult;
		...
		if(lBattlegroundInfo_obj_arr)
		{
			...
			for( let i = 0; i < lBattlegroundInfo_obj_arr.length; i++ )
			{
				...
				{
					if(lData_obj.seats[j].id === lSeatId_int)
					{
						lData_obj.seats[j].battlegroundInfo = l_obj;

						if(lData_obj.seats[j].nickname === this._fPlayerInfo_gpi.nickname)
						{
		...
		this.info.totalDamage = (lBattlegroundPlayerInfo_obj ? lBattlegroundPlayerInfo_obj.score : lData_obj.totalDamage) || 0;
		
		this.info.totalKillsCount = lData_obj.enemiesKilledCount || 0;
		this.info.totalFreeShotsCount = lData_obj.freeShotsWon || 0;
		let bulletsFired = lData_obj.bulletsFired || 0;
		let playerWonValue = lData_obj.realWinAmount || 0;
		...
		if (lBattlegroundPlayerInfo_obj && lData_obj.battlegroundRoundResult)
		{
			...
			else
			{
				...
				let lWinnerSeatId_num = parseInt(lData_obj.battlegroundRoundResult[0].id);

				for (let i = 0;  i < lData_obj.seats.length; i++)
				{
					if (parseInt(lData_obj.seats[i].id) == lWinnerSeatId_num)
					{
						lWinnerNickname_str = lData_obj.seats[i].nickname;
						break;
					}
				}
		...
		{
			lListData_arr = this._sortSeatsByBattlegroundRank(lData_obj.seats);
		}
		else
		{
			lListData_arr = this._sortSeatsByDamage(lData_obj.seats);
		}

		this.info.listData = lListData_arr;

		this.info.questsCompletedCount = lData_obj.questsCompletedCount || 0;
		this.info.questsPayouts = lData_obj.questsPayouts || 0;

		this.info.roundId = lData_obj.roundId;
		this.info.roundResultResponseRecieved = true;

		this.info.weaponsSurplus = 0;
		if (lData_obj.weaponSurplus && lData_obj.weaponSurplus.length > 0)
		{
			let lPayout_num = 0;

			for (let i = 0; i < lData_obj.weaponSurplus.length; ++i)
			{
				lPayout_num += lData_obj.weaponSurplus[i].winBonus;
			}
	...
	_onBattlegroundTimeToStartUpdated(event)
	{
		...
		clearInterval(this._fTimeInterval);

		if (this.info.isScreenActive)
		{
			this._fTimeInterval = setInterval(this._tick.bind(this), 100);
			this.info.setTimeToStart(event.timeToStart);
			this.view.battleGroundTimeToStartUpdated(this.info.getFormattedTimeToStart(true));
			this._fIsTimeToStartUpdateExpeted_bl = false;
		}
	}

	_tick()
	{	
		let lFormattedTime = this.info.getFormattedTimeToStart(true);

		if (lFormattedTime)
		{
			this.view.battleGroundTimeToStartUpdated(lFormattedTime);
		}
		else
		{
			clearInterval(this._fTimeInterval);
			this.view.battleGroundTimeToStartUpdated("--:--:--");

			if (this.info.isScreenActive
				&& !this._fIsTimeToStartUpdateExpeted_bl)
			{
				this._fIsTimeToStartUpdateExpeted_bl = true;
				this.emit(RoundResultScreenController.EVENT_ON_BATTLEGROUND_ROUND_RESULT_TIMER_EXPIRED);
			}
		}
	}
	...
	{
		if (APP.isDialogActive(GUSDialogsInfo.DIALOG_ID_NO_WEAPONS_FIRED))
		{
			return;
		}
		...
		if (APP.isBattlegroundGame && APP.gameScreen.isScoreboardAnimationsPlaying)
		{
			// https://jira.dgphoenix.com/browse/MQPRT-375
			this._fActivationTimer_t && this._fActivationTimer_t.destructor();
			this._fActivationTimer_t = new Timer(()=>this._showScreenImmediately(aSkipAnimation_bl, aRoundResultOnAwaitingGameUnpause), 120);
		}
		else
		{
	...
	_showScreenAfterTransitionIntro(aSkipTransitionIntro_bl = false)
	{
		if (this._fIsNeedShowScreenAfterTransitionIntro_bl || aSkipTransitionIntro_bl)
		{
			...
			if (l_rrsv)
			{
				...
				if(APP.isBattlegroundGame)
				{
					this.emit(RoundResultScreenController.EVENT_ON_ROUND_RESULT_SCREEN_ACTIVATED, {roundResultOnAwaitingGameUnpause: this._fIsRoundResultOnAwaitingGameUnpause_bl, needStartSummary: false});
					l_rrsv.on(BattlegroundResultScreenView.ON_MAIN_PANEL_ANIMATION_COMPLITE, () => {this.emit(RoundResultScreenController.EVENT_ON_ROUND_RESULT_SCREEN_ACTIVATED, {roundResultOnAwaitingGameUnpause: this._fIsRoundResultOnAwaitingGameUnpause_bl, needStartSummary: true});}, this)
				}
				else
				{
	...
	_hideScreen()
	{
		...
		if (APP.isBattlegroundGame)
		{
			this._fGameScreen_gs.off(GameScreen.EVENT_BATTLEGROUND_TIME_TO_START_UPDATED, this._onBattlegroundTimeToStartUpdated, this, true);
		}
