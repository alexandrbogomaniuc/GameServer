...
class Game extends GUSGameApplication
{
	...
	constructor(...args)
	{
		...
		this._fIsGameStartUrlOnGameStartClickAfterAssetsLoadingCompletUpdateExpected_bl = null;
	...
	_onTexturesInited()
	{
		...
		{
			this._fIsGameStartUrlOnGameStartClickAfterAssetsLoadingCompletUpdateExpected_bl = true;
		}
	...
	{
		if (this.tournamentModeController.info.isTournamentOnClientCompletedState || this._fIsGameStartUrlOnGameStartClickAfterAssetsLoadingCompletUpdateExpected_bl)
		{
			return;
		}
	...
	onLobbyMessageReceived(event)
	{
		...
		switch (msgType)
		{
			...
			case LOBBY_MESSAGES.GAME_URL_UPDATED:
				if (!event.data.isBattleground)
				{
					break;
				}
				
				this._fIsGameStartUrlOnGameStartClickAfterAssetsLoadingCompletUpdateExpected_bl = false;
				
				if (event.data.isNeedReconnectOnChange)
				{
					break;
				}

				let updatedGameUrl = event.data.gameUrl;
				this.startServerInteraction(updatedGameUrl);
				break;
			...
			case LOBBY_MESSAGES.END_OF_GAME_REFUND:
				this._endOfRoundRefund = event.data.refund;
				break;

		}
	}

	get endOfRoundRefund()
	{
		return this._endOfRoundRefund;
	}

	set endOfRoundRefund(value)
	{
		this._endOfRoundRefund = value;
	}
