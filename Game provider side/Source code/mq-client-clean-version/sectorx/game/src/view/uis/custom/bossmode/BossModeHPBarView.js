...
class BossModeHPBarView extends SimpleUIView
{
	...
	_initBarContainer()
	{
		...
		this._fBarContainer_sprt.position.set(APP.isMobile ? 10 : -35, -5);
	}
	...
	_createBarView()
	{
		...
		this._fProgressBar_sprt.scale.set(2);
		this._fProgressBar_sprt.position.set(-lBarTexture_t.width, -lBarTexture_t.height);
		...
		this._fFrame_sprt.scale.set(1, 0.97);
	}
	...
	_showHPBar(aSkipAnimation_bl = false)
	{
		...
		if ( APP.currentWindow.freezeCapsuleFeatureController.info.freezeTime > 500 && !APP.isMobile)
		{
			this.position.x += 40;
		}
	...
	_updateBar()
	{
		...
		this._fProgreesBarMask_g.position.set(0, 2*this._fProgressBar_sprt.texture.height*(1-this.uiInfo.progressHealth));
	}
