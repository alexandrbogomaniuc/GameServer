...
class LightningCapsuleFeatureView extends SimpleUIView
{
	...
	{
		let lIsAnimation_bl = false;

		this._fHitLightningAnimations_spr_arr && this._fHitLightningAnimations_spr_arr.forEach(
			function(el, index) 
			{
				lIsAnimation_bl = true;
			}
		);

		this._fBossHitLightningAnimations_spr_arr && this._fBossHitLightningAnimations_spr_arr.forEach(
			function(el, index) 
			{
				lIsAnimation_bl = true;
			}
		...
		this._fBossGlowIndigo_spr_arr && this._fBossGlowIndigo_spr_arr.forEach(
			function(el, index) 
			{
				lIsAnimation_bl = true;
			}
		...
		this._fGlowIndigo_spr_arr && this._fGlowIndigo_spr_arr.forEach(
			function(el, index)
			{
				lIsAnimation_bl = true;
			}
	...
	_onLightningCapsuleEnemyHitNeeded(e)
	{
		...
		this.emit(LightningCapsuleFeatureView.EVENT_ON_TARGET_HIT, { hitInfo: this._fHitObj_arr[e.enemyId]});
	}
	...
	_onLightningCapsuleEnemyHitAnimationCompleted(e)
	{
		if (e.typeId === 100)
		{
			const lIndex_int = this._fBossHitEnemyAnimations_spr_arr.indexOf(e.target);
			if (~lIndex_int)
			{
		...
		{
			const lIndex_int = this._fHitEnemyAnimations_spr_arr.indexOf(e.target);
			if (~lIndex_int)
			{
		...
		let lCapsuleEnemyId_num = e.capsuleId;
	...
	_tryToFinishAnimation(aCapsuleEnemyId_num)
	{
		...
		{
			this.emit(LightningCapsuleFeatureView.EVENT_ON_ALL_ANIMATIONS_COMPLETED);
		}
	}
