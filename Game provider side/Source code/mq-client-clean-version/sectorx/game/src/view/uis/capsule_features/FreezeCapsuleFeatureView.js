...
class FreezeCapsuleFeatureView extends SimpleUIView
{
	...
	i_updateTimerForBossHPBar(aDoesBossExist_bl)
	{
		...
		let lTimerContainerYPostion_num = APP.isMobile ? 375 : 385;
	...
	constructor()
	{
		...
		this._fProgressBarOverlap_g = null;
		this._fProgressBarCap_g = null;
	...
	_initTimerContainer()
	{
		...
		// PROGRESS BAR...
		let lProgressBar_sprt = this._fProgressBar_sprt = this._fTimerContainer_spr.addChild(APP.library.getSprite("enemies/freeze_capsule/vertical_freeze_timer_progress_bar"));
		...
		this._fProgressBarOverlap_g = lProgressBarOverlapContainer_sprt.addChild(new PIXI.Graphics());
		this._fProgressBarCap_g = lProgressBarOverlapContainer_sprt.addChild(new PIXI.Graphics());
		// ...PROGRESS BAR OVERLAP
	...
	{
		if (!this._fTimerContainer_spr || !this._fProgressBarOverlap_g || aOptValue_num < 0)
		{
			return;
		}

		let lProgressBar_sprt = this._fProgressBar_sprt;

		let lOverlapHeight_num = lProgressBar_sprt.height * (1 - aOptValue_num);

		let lOverlap_g = this._fProgressBarOverlap_g;
		lOverlap_g.clear();
		...
		this._fProgressBarCap_g.clear();
		let lOverlapCap_g = this._fProgressBarCap_g;
