...
import { GAME_ROUND_STATE } from '../../../../../missionamazon/shared/src/CommonConstants';
...
class MainPlayerSpot extends PlayerSpot
{
	...
	_init()
	{
		...
		if(this._isBottom){
			this.nickname_tf.y = this.nickname_tf.y - 2;
			this._fStakeValue_tf.y = this._fStakeValue_tf.y - 1;
		}else
		{
			this.nickname_tf.y = this.nickname_tf.y + 0.8;
			this._fStakeValue_tf.y = this._fStakeValue_tf.y + 1.5;
		}		
	}
	...
	{
		this._validateButtonsEnable();
		if(event.value == GAME_ROUND_STATE.PLAY){
			this._isPlusBetLevelChangeRequiredAfterFiring_bl = false;
			this._isMinusBetLevelChangeRequiredAfterFiring_bl = false;
		}
	}
	...
	_invalidateShotPrice()
	{
		...
		let finalNumber = APP.currencyInfo.i_formatNumber(lBet_num, true);
		if(finalNumber.indexOf(".00 ")>-1){
			finalNumber = finalNumber.replace(".00","");
		}
		this._fStakeValue_tf.text = finalNumber; 
	}
	...
	_addBack()
	{
		this._fBack_sprt = this.addChild(APP.library.getSpriteFromAtlas("player_spot/ps_player_spot/back"));
	...
	_addNickname()
	{
		...
		lNickname_tf.position.set(-46.2, 21.5);

		if (!this._isBottom)
		{
			lNickname_tf.position.set(-46.2, -19.5);
		}
	}
