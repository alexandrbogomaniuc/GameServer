...
class LevelUpAnimation extends Sprite
{
	...
	_addFlares()
	{
		...
		this._addFlare({x: 0, y: 0}, {from:0.3490658503988659, to:0.6981317007977318}, {from:0.6, to:0}); //Utils.gradToRad(20)  Utils.gradToRad(40)
		
		if (this.isPowerUpAnimation)
		{
			let lYMult = this._fIsBottomSpot_bl ? 1 : -1;
			this._addFlare({x: 0+5, y: 10*lYMult}, {from:0.17453292519943295, to:0.5235987755982988}, {from:0.35, to:0}); //Utils.gradToRad(10)  Utils.gradToRad(30)
			this._addFlare({x: 0-5, y: -35*lYMult}, {from:0.17453292519943295, to:0.5235987755982988}, {from:0.35, to:0}); //Utils.gradToRad(10)  Utils.gradToRad(30)
			this._addFlare({x: 0, y: 35*lYMult}, {from:0.17453292519943295, to:0.5235987755982988}, {from:0.35, to:0}); //Utils.gradToRad(10)  Utils.gradToRad(30)
		}
		else
		{
			this._addFlare({x: 0-5, y: 0}, {from:0.17453292519943295, to:0.5235987755982988}, {from:0.35, to:0}); //Utils.gradToRad(10)  Utils.gradToRad(30)
			this._addFlare({x: 0-3, y: 0-5}, {from:0.17453292519943295, to:0.5235987755982988}, {from:0.35, to:0}); //Utils.gradToRad(10)  Utils.gradToRad(30)
		}
	}
	...
	_addFlare(aPos_obj, aAngle_obj, aScale_obj)
	{
		...
		lFlare_spr.rotation = aAngle_obj.from;

		lFlare_spr.rotateTo(aAngle_obj.to, 15*FRAME_RATE);
	...
	_addYellowSmokes()
	{
		...
		this._addYellowSmoke(-1.3962634015954636, {x: -0.75, y: 0.75}); //Utils.gradToRad(-80)
		this._addYellowSmoke(1.8325957145940461, {x: 0.75, y: 0.75}); //Utils.gradToRad(105)
	...
	_addYellowSmoke(aAngle_num, aScale_obj)
	{
		...
		lSmoke_spr.rotation = aAngle_num;
	...
	{
		return APP.gameScreen.gameFieldController.spot.position;
	}
