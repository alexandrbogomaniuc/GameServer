...
class CursorAnimation extends Sprite
{
	...
	_init()
	{
		...
		this._rotationContainer_sprt.rotateTo(Math.PI*2, 60*FRAME_RATE, undefined, undefined, undefined, true); // rotation loop
	}
	...
	_updatePosition(mousePos)
	{
		if (this._container_sprt)
		{
			...
			this._container_sprt.position.set(mousePos.x-lAPPSize_obj.width/2, mousePos.y-lAPPSize_obj.height/2);
		}
	}
	...
	_startRender()
	{
		...
		{
			this._cursorOuterView_sprt = this._rotationContainer_sprt.addChild(APP.library.getSpriteFromAtlas("cursor/CursorCrosshair_outer"));
		}

		if (!this._cursorView_sprt)
		{
			this._cursorView_sprt = this._container_sprt.addChild(APP.library.getSpriteFromAtlas("cursor/CursorCrosshair"));
		}
