...
import { generateFlameTextures } from '../animation/boss_mode/fire/FlameAnimation';

function _generateTurret3FlameExplosionTextures()
{
...
const WIN_FLARE_PARAM =[
	...
	},
	{
		delay: 10,
		duration: 7,
		position: {x: 8.5, y: -21.5875}, //x: 68 / 8, y: -172.7 / 8
		rotation: 0.8028514559173915, //Utils.gradToRad(46)
		scale: {x: 0.87, y: 0.87},
...
class MissEffectLevel3ExplosionAnimation extends Sprite
{
	...
	{		
		for (let i = 0; i < WIN_BLAST_PARAM.length; i++)
		{
			...
			let lWinBlastTier_spr = this._fWinBlastTier_spr_arr[i] = this.addChild(APP.library.getSpriteFromAtlas('enemy_impact/turret_3/win_blast_tier'));
	...
	_startBlast()
	{
		...
		let lBlast1_spr = this._fBlast1_spr = this._fBlastContainer_spr.addChild(APP.library.getSpriteFromAtlas('enemy_impact/turret_3/blast'));
		let lBlast2_spr = this._fBlast2_spr = this._fBlastContainer_spr.addChild(APP.library.getSpriteFromAtlas('enemy_impact/turret_3/blast'));
		lBlast1_spr.alpha = 0;
	...
	_startWinFlare()
	{
		for (let i = 0; i < WIN_FLARE_PARAM.length; i++)
		{
			...
			let lWinFlare_spr = this._fWinFlare_spr_arr[i] = this.addChild(APP.library.getSpriteFromAtlas('enemy_impact/turret_3/win_flare'));
	...
	_startLightParticle()
	{
		let lLight1_spr = this._fLightParticle1_spr = this._fContainerLights1_spr.addChild(APP.library.getSpriteFromAtlas('enemy_impact/turret_3/light_particle'));
		lLight1_spr.position.set(-2, -18);
		lLight1_spr.alpha = 0;
		lLight1_spr.scale.set(9.12, 9.12);
		...
		let lLight2_spr = this._fLightParticle2_spr = this._fContainerLights2_spr.addChild(APP.library.getSpriteFromAtlas('enemy_impact/turret_3/light_particle'));
		lLight2_spr.position.set(-2, -18);
		lLight2_spr.alpha = 0;
		lLight2_spr.scale.set(9.64, 9.64);
	...
	_startLight3()
	{
		let lLight31_spr = this._fLight31_spr = this._fContainerLights1_spr.addChild(APP.library.getSpriteFromAtlas('enemy_impact/turret_3/light_3'));
		...
		let l1_seq = [
			{tweens: [], duration: 1 * FRAME_RATE},
			{tweens: [{prop: 'alpha', to: 1}], duration: 0 * FRAME_RATE},
			{tweens: [{prop: 'scale.x', to: 1.6}, {prop: 'scale.y', to: 1.6}], duration: 1 * FRAME_RATE},
		...
		let lLight32_spr = this._fLight32_spr = this._fContainerLights2_spr.addChild(APP.library.getSpriteFromAtlas('enemy_impact/turret_3/light_3'));
	...
	{
		let lFlameExplosion_spr = this._fFlameExplosion_spr = this.addChild(new Sprite());
		lFlameExplosion_spr.textures = _generateTurret3FlameExplosionTextures();
		lFlameExplosion_spr.animationSpeed = 0.5; // 30 / 60;
		lFlameExplosion_spr.position.x = 6.1; //12.2 / 2;
		lFlameExplosion_spr.position.y = -2.6; //-5.2 / 2;
		lFlameExplosion_spr.alpha = 0.5;
		lFlameExplosion_spr.blendMode = PIXI.BLEND_MODES.ADD;
		lFlameExplosion_spr.scale.set(0.3);
		this._fAnimationCount_num++;
		lFlameExplosion_spr.play();
		lFlameExplosion_spr.on('animationend', ()=>{
			lFlameExplosion_spr && lFlameExplosion_spr.destroy();
			this._fAnimationCount_num--;
			this._onAnimationCompletedSuspicison();
		});
	}	
	...
	_startCircleBlast()
	{
		let lCircleBlast_spr = this._fCircleBlast_spr = this.addChild(APP.library.getSpriteFromAtlas('enemy_impact/turret_3/circle_blast'));
		...
		let l_seq = [
			...
			{tweens: [{prop: 'scale.x', to: 1.64}, {prop: 'scale.y', to: 1.64}], duration: 2 * FRAME_RATE},
			{tweens: [{prop: 'scale.x', to: 2.048}, {prop: 'scale.y', to: 2.048}, {prop: 'alpha', to: 0}], duration: 4 * FRAME_RATE,
				onfinish: ()=>{
	...
	_onAnimationCompletedSuspicison()
	{
		...
		{
			this.emit(MissEffectLevel3ExplosionAnimation.EVENT_ON_ANIMATION_ENDED);
		}
	}
...
export default MissEffectLevel3ExplosionAnimation;