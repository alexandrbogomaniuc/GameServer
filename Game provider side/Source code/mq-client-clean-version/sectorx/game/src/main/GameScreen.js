...
class GameScreen extends Sprite
{
	...
	static get EVENT_ON_BET_MULTIPLIER_UPDATE_REQUIRED()					{return "EVENT_ON_BET_MULTIPLIER_UPDATE_REQUIRED";}
	...
	static get EVENT_ON_BATTLEGROUND_CONFIRM_NEXT_ROUND_UPDATED()			{return "EVENT_ON_BATTLEGROUND_CONFIRM_NEXT_ROUND_UPDATED"; }	
	...
	init(aOptRoomUrl_str=undefined)
	{
		...
		this._initScoreboardController();
		...
		this.fireSettingsController.on(FireSettingsController.EVENT_ON_FIRE_SETTINGS_CHANGED, this._onFireSettingsChanged, this);
	...
	_onPlayerBetMultiplierUpdateRequired(aEvent_obj)
	{
		...
		this.emit(GameScreen.EVENT_ON_BET_MULTIPLIER_UPDATE_REQUIRED, {id: aEvent_obj.id, multiplier: aEvent_obj.multiplier});
	}
		...
		switch (msgType)
		{
			...
			case LOBBY_MESSAGES.DIALOG_DEACTIVATED:
				...
				if(
					...
					event.data.dialogId === GUSDialogsInfo.DIALOG_ID_INSUFFICIENT_FUNDS
					)
				{
			...
			case LOBBY_MESSAGES.BATTLEGROUND_PLAY_AGAIN_CONFIRMED:
				if (APP.isBattlegroundGame && !this.gameStateController.info.isPlayerSitIn)
				{
	...
	_initPlayersSpotsController()
	{
		...
		l_psc.on(PlayerSpotsController.EVENT_ON_BET_UPDATE_REQUIRED, this._onPlayerBetMultiplierUpdateRequired, this);
	...
	_onCoinLanded(aEvent_obj)
	{
		if (aEvent_obj.money)
		{
			if (aEvent_obj.seatId === APP.playerController.info.seatId)
			{
				...
				this.balanceController.updatePlayerWin(aEvent_obj.money, null);
			}
			else
			{
	...
	_initTransitionViewController()
	{
		...
		let l_tvc = this._fTransitionViewController_tvc = APP.isBattlegroundGame ? new BTGTransitionViewController(): new TransitionViewController();
		l_tvc.init();
		return l_tvc;
	}
	//...TRANSITION VIEW
	...
	_initGameplayDialogsController()
	{
		...
		l_gdsc.gameBattlegroundCountDownDialogController.on(BattlegroundCountDownDialogController.EVENT_BATTLEGROUND_COUNTDOWN_CANCEL_CLICKED, this._onCountDownCancelClicked, this);
		return l_gdsc;
	}
	//...GAMEPLAY DIALOG

	_onCountDownCancelClicked()
	{
	...
	{
		this.gameFieldController.onFireSettingsChanged();
	}
	...
	_initTargetingController()
	{
		...
		l_tc.on(TargetingController.EVENT_ON_TARGET_RESET, this._tryToResumeAutofire, this)
		return l_tc;
	}
	...
	_tryToResumeAutofire()
	{
		...
		if(APP.isBattlegroundGame && this.fireController.autofireButtonEnabled) return;
	...
	{
		if (APP.isBattlegroundGame && this.gameFieldController.spot && this.gameFieldController.spot.autofireButton.enabled != aState_bl)
		{
			this.fireController.autofireButtonEnabled = aState_bl;
			this.gameFieldController.spot.autofireButton.i_setEnable(aState_bl, aEnemy_e, aOptIsQuietMode_bl);
		}
	}
	//...FIRE
	...
	{
		this.gameFieldController.roundResultScreenController.on(RoundResultScreenController.EVENT_ON_ROUND_RESULT_SCREEN_ACTIVATED , this._onRoundResultActivated, this);
		this.gameFieldController.roundResultScreenController.on(RoundResultScreenController.EVENT_ON_BATTLEGROUND_NEXT_ROUND_CLICKED, this._onBattlegroundNextRoundClicked, this);
		this.gameFieldController.roundResultScreenController.on(RoundResultScreenController.EVENT_ON_BATTLEGROUND_ROUND_RESULT_TIMER_EXPIRED, this._onRoundResultTimerExpired, this);
	...
	_onTimeToShowTutorial()
	{
		if (this.gameFieldController.spot)
		{
			APP.externalCommunicator.sendExternalMessage(GAME_MESSAGES.TIME_TO_SHOW_TUTORIAL, {
				...
				dragonStonePanel: 	this.gameFieldController.spot.getBounds(),
	...
	_onRoundResultTimerExpired()
	{
		if (APP.isDialogActive(GUSDialogsInfo.DIALOG_ID_BATTLEGROUND_BUY_IN_CONFIRMATION))
		{
			return;
		}
	...
	_onCancelBattlegroundRound(aEvent_obj)
	{
		...
		APP.externalCommunicator.sendExternalMessage(GAME_MESSAGES.BATTLEGROUND_ROUND_CANCELED, {isGameInProgress: l_gsi.isGameInProgress, isQualifyState: l_gsi.isQualifyState, refundedAmount: aEvent_obj.messageData.refundedAmount });
		this.emit(GameScreen.EVENT_ON_BATTLEGROUND_ROUND_CANCELED, {isGameInProgress: l_gsi.isGameInProgress, isQualifyState: l_gsi.isQualifyState, refundedAmount: aEvent_obj.messageData.refundedAmount });
	...
	getRoomInfoResponse(data)
	{
		...
		if(data.battlegroundInfo)
		{
			...
			APP.gameScreen.gameFieldControllerController.roundResultScreenController.info.battlegroundBuyIn = this._fbattlegroundBuyInCost_num;
	...
	_requestFullGameInfo()
	{
		...
		if (
			...
			&&
				(
					APP.isDialogActive(GUSDialogsInfo.DIALOG_ID_FRB) ||
					APP.isDialogActive(GUSDialogsInfo.DIALOG_ID_BONUS)
				)
		)
		{
			return;
