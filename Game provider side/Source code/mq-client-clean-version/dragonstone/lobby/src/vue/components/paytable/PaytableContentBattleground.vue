		...
		methods: {
			...
			_updateWeaponSliderPositionIfRequired(aPageName_str)
			{
					...
					)
				{
					...
					switch(this._getPageId(aPageName_str))
					{
						case "dragon":
							lTargetPositionDescriptor_html = lBossPositionDescriptor_html;
							break;
					}
			...
			updateContent(page)
			{
				...
				if(
					...
					this._getPageId(page) == "spirit_enemies" ||
					this._getPageId(page) == "dragon"
					)
				{
				...
				//FRAGMENTS...
				this._replaceAll(
					"##DRAGONSTONE_FRAGMENTS_COUNT##",
					INDICATORS_CONSTANT_VALUES.DRAGONSTONE_FRAGMENTS_COUNT
					);
				//...FRAGMENTS
				...
				//SLOTS MULTIPLIERS...
				let lMultipliers_int_arr = INDICATORS_CONSTANT_VALUES.SLOTS_WIN_MULTIPLIERS;
				for( let i = 0; i < lMultipliers_int_arr.length; i++ )
				{
					this._replaceAll(
						"##SLOTS_WIN_MULTIPLIERS_" + i + "##",
						lMultipliers_int_arr[i]
						);
				}
				//...SLOTS MULTIPLIERS
