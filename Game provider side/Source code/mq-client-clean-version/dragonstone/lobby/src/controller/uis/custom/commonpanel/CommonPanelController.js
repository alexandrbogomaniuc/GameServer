...
class CommonPanelController extends SimpleUIController
{
	...
	_onFinalCounting(event){
		if (!APP.isMobile) return;
		this.view.disableMobileMenuGroup();
	}

	_onFinalCountingCompleted(event)
	{
		if (!APP.isMobile) return;
		this.view.enableMobileMenuGroup();
	}
	...
	_onGameMessageReceived(event)
	{
		...
		switch (msgType)
		{
			...
			case GAME_MESSAGES.REFRESH_COMMON_PANEL_REQUIRED:
				...
				break;
			case GAME_MESSAGES.FINAL_COUNTDOWN_COMPLETED:
				this._onFinalCountingCompleted();
			break;
			case GAME_MESSAGES.FINAL_COUNTDOWN_STARTED:
				this._onFinalCounting();
			break;
		}
	}
