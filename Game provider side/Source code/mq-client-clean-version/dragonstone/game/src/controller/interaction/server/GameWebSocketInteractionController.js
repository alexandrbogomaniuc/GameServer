...
class GameWebSocketInteractionController extends WebSocketInteractionController {
	...
	_establishConnection() {
		...
		// FIX: Override URL here in the Game Controller to ensure it applies
		if (this.info.socketUrl && this.info.socketUrl.indexOf("games.local") > -1) {
			console.log("[GameWSIC] internal hostname detected. Redirecting to localhost:80 (Nginx)");
			this.info.socketUrl = this.info.socketUrl.replace("games.local", "localhost:80");
			console.log("[GameWSIC] New URL:", this.info.socketUrl);
		}

		// FIX: Ensure parameters are present (Critical for "Null key" error)
		console.log("[GameWSIC] Verifying Code Version: 2.6 (Port 80 + lowercase 'sid')");
		try {
			if (this.info.socketUrl && this.info.socketUrl.indexOf("?") === -1) {
				console.log("[GameWSIC] URL missing parameters. Inspecting sources...");
				...
				const getParam = (key) => {
					// 1. Try APP
					if (typeof APP !== 'undefined' && APP.urlBasedParams) {
						if (APP.urlBasedParams[key]) return APP.urlBasedParams[key];
						let keys = Object.keys(APP.urlBasedParams);
						for (let k of keys) {
							if (k.toLowerCase() === key.toLowerCase()) return APP.urlBasedParams[k];
						}
					}
					// 2. Try Window Search
					try {
						if (window && window.location && window.location.search) {
							let match = new RegExp('[?&]' + key + '=([^&]*)', 'i').exec(window.location.search);
				...
				// Critical Params extraction
				const sid = getParam("SID") || getParam("sid");
				if (sid) {
					// V2.6: Use lowercase 'sid'
					finalParams.push("sid=" + sid);
					foundSid = true;
				} else {
					console.error("[GameWSIC] CRITICAL: Could not find SID in APP or Window Config!");
				}

				// Context Params
				const keysToInject = ["gameId", "roomId", "lang", "serverId", "GAMESERVERID", "BANKID", "mode"];
				keysToInject.forEach(k => {
					let v = getParam(k);
					if (v) finalParams.push(k + "=" + v);
				});

				if (finalParams.length > 0) {
					this.info.socketUrl += "?" + finalParams.join("&");
					console.log("[GameWSIC] Appended params. Success: " + foundSid + ". Final URL:", this.info.socketUrl);
				} else {
					console.warn("[GameWSIC] Could not find ANY params to inject.");
				}
			}
		} catch (e) {
			console.error("[GameWSIC] Error injecting params:", e);
		}
	...
	_sendOpenRoomRequest() {
		// V2.8 FIX: Ultra-Robust Param Extraction
		const getParam = (key) => {
			let val = null;
			// 1. Try APP.urlBasedParams (if exists)
			if (typeof APP !== 'undefined' && APP.urlBasedParams) {
				// Exact match
				if (APP.urlBasedParams[key]) val = APP.urlBasedParams[key];
				// Case-insensitive match
				if (!val) {
					let keys = Object.keys(APP.urlBasedParams);
					for (let k of keys) {
						if (k.toLowerCase() === key.toLowerCase()) {
							val = APP.urlBasedParams[k];
							break;
						}
					}
				}
			...
			// Falsy check (empty string, null, undefined)
			if (!val || val === "undefined" || val === "null") return null;
			return val;
		};

		let lRoomId_num = getParam("roomId");
		let lServerId_num = getParam("serverId");
		// Check ALL possible variations of SID
		let lSid = getParam("SID") || getParam("sid") || getParam("sessionId") || getParam("token");

		console.log("[GameWSIC] _sendOpenRoomRequest: SID=" + lSid + ", Room=" + lRoomId_num);

		this._onLobbyMessageReceived({ type: LOBBY_MESSAGES.GAME_REFRESH_BALANCE_REQUIRED })

		this._fOpenRoomSent_bln = true;
		// V2.7: Pass extracted variables
		this._sendRequest(CLIENT_MESSAGES.OPEN_ROOM, { sid: lSid, serverId: lServerId_num, roomId: lRoomId_num, lang: I18.currentLocale });
	}
