...
class RoundResultScreenController extends SimpleUIController {
	...
	_showScreen(aSkipAnimation_bl = false, aRoundResultOnAwaitingGameUnpause = false) {
		...
		setTimeout(()=>{
			if (APP.isBattlegroundGame && APP.gameScreen.gameField.isScoreboardAnimationsPlaying || (!APP.currentWindow.isKeepSWModeActive || APP.currentWindow.bossModeController.isBossSequenceInProgress)) {

				// https://jira.dgphoenix.com/browse/MQPRT-375
				const destructionTime = APP.currentWindow.bossModeController.isBossSequenceInProgress || APP.currentWindow.bossModeController.isWinPresentationAwaiting? APP.currentWindow.bossModeController.bossExtentionTime : 120;
				this._fActivationTimer_t && this._fActivationTimer_t.destructor();
				this._fActivationTimer_t = new Timer(() => this._showScreenImmediately(aSkipAnimation_bl, aRoundResultOnAwaitingGameUnpause), destructionTime);
			}
			else {
