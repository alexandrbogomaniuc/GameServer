...
export const HIT_RESULT_MINI_SLOT_WIN_ID = 4;
...
class PrizesController extends SimpleController {
	...
	static get i_EVENT_ON_TIME_TO_SHOW_MINI_SLOT_PRIZE() 			{ return 'EVENT_ON_TIME_TO_SHOW_MINI_SLOT_PRIZE';}
			...
			for (let lSeatId_int in aHitData_obj.hitResultBySeats)
			{
				...
				for (let lPrize_obj of lPrizesForSeat_obj_arr)
				{
					switch (lPrize_obj.id)
					{
						...
						case HIT_RESULT_MINI_SLOT_WIN_ID:
	...
	_onTimeToShowPrizes(event)
	{
		...
		if (hitData.slot && this._getSlotWins(hitData.slot) > 0)
		{
			// if already exists (TODO to server to remove)
			let prizes = hitData.hitResultBySeats[hitData.seatId];
			let lIsMiniSlotPrizeAlreadyExist_bl = false;
			for (let i=0; i<prizes.length; i++)
			{
				let prize = prizes[i];
				if (prize.id === HIT_RESULT_MINI_SLOT_WIN_ID)
				{
					lIsMiniSlotPrizeAlreadyExist_bl = true;
					break;
				}
			}

			if (!lIsMiniSlotPrizeAlreadyExist_bl)
			{
				 if (!hitData.hitResultBySeats[hitData.seatId])
				{
					hitData.hitResultBySeats[hitData.seatId] = [];
				}
				hitData.hitResultBySeats[hitData.seatId].push({id: HIT_RESULT_MINI_SLOT_WIN_ID, value: this._getSlotWins(hitData.slot)});
			}
		}
		...
		for (let lSeatId_int in hitData.hitResultBySeats)
		{
			...
			for (let lPrize_obj of lPrizesForSeat_obj_arr)
			{
				switch (lPrize_obj.id)
				{
					...
					case HIT_RESULT_SPECIAL_WEAPON_ID:
						...
						if (isNaN(lSWPrizeValue) || lSWPrizeValue === null)
						{
						}
						else
						{
							lSpecialWeaponIds_int_arr.push({id: +lSWPrizeValue, nextBetLevel: hitData.nextBetLevel});
						}
						break;
					case HIT_RESULT_MINI_SLOT_WIN_ID:
						if (lIsMasterSeat_bl)
						{
							lMiniSlotValue_num = +lPrize_obj.value;
						}
						else
						{
			...
			if (!lIsMasterSeat_bl && lSpecialWeaponIds_int_arr.length > 0)
			{
				...
				for (let i = 0; i < lSpecialWeaponIds_int_arr.length; i++)
				{
					...
					this.emit(PrizesController.i_EVENT_ON_TIME_TO_SHOW_SPECIAL_WEAPON_PRIZE, {specialWeaponId: lSpecialWeaponIds_int_arr[i].id, nextBetLevel: lSpecialWeaponIds_int_arr[i].nextBetLevel, seatId: +lSeatId_int, startPosition: position});
				}
			}
			//...SPECIAL WEAPONS
		}

		if (hitData.slot && hitData.seatId == APP.gameScreen.gameField.seatId)
		{
			this.emit(PrizesController.i_EVENT_ON_TIME_TO_SHOW_MINI_SLOT_PRIZE, {hitData: hitData, value: lMiniSlotValue_num});
		}
