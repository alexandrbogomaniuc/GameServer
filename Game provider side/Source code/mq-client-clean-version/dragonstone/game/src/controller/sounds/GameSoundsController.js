...
import { ENEMIES, ENEMY_TYPES , BACKGROUND_SOUNDS_FADING_TIME } from '../../../../shared/src/CommonConstants';
...
import FragmentsPanelController from '../uis/dragonstones/FragmentsPanelController';
import FragmentsController from '../uis/dragonstones/FragmentsController';
...
let BG_SOUNDS = [
	...BG_MAIN_SOUNDS,
	"mq_mus_game_bg_prev_boss",
	"mq_mus_game_bg_boss",
	"mq_dragonstone_orc_music",
...
class GameSoundsController extends SoundsController
{
	...
	constructor()
	{
		...
		this._fLastRavenSoundCallTime_num = undefined;
		this._fCurrentRavenCryCount_int = 0;
		this._fLastRavenSoundCallTime_num = undefined;
		this._fRecentRavenSoundCallCount_int = 0;
	...
	initListeners(aGameScreen_gs)
	{
		...
		aGameScreen_gs.on(GameScreen.EVENT_ON_GARGOYLE_FLAPPING_WINGS, this._onGargoyleFlappingWings, this);
		aGameScreen_gs.on(GameScreen.EVENT_ON_DRAGON_FLAPPING_WINGS, this._onDragonFlappingWings, this);
		aGameScreen_gs.on(GameScreen.EVENT_ON_DRAGON_FIRE_BREATH_STARTED, this._onDragonFireBreathStarted, this);
		aGameScreen_gs.on(GameScreen.ON_FRAGMENT_LANDED, this._onFragmentLanded, this);
		aGameScreen_gs.on(GameScreen.EVENT_ON_SOME_ENEMY_SPAWN_SOUND_REQUIRED, this._onSomeEnemySpawnSoundRequired, this);
		aGameScreen_gs.on(GameScreen.EVENT_ON_RAGE_SOUND_REQUIRED, this._onSomeEnemyRageSoundRequired, this);
		aGameScreen_gs.on(GameScreen.EVENT_ON_MINI_SLOT_OCCURED, this._onMinislotOccured, this);
		aGameScreen_gs.on(GameScreen.EVENT_ON_MINI_SLOT_OUTRO, this._onMinislotOutro, this);
		aGameScreen_gs.on(GameScreen.EVENT_ON_MINI_SLOT_SPIN_STARTED, this._onMinislotSpin, this);
		...
		let lGameField_gf = APP.gameScreen.gameField;

		lGameField_gf.on(GameField.EVENT_ORCS_PROCESSION_STARTED, this._onOrcsProcessionStarted, this);
		lGameField_gf.on(GameField.EVENT_ORCS_PROCESSION_FINISHED, this._onOrcsProcessionFinished, this);
		...
		lBigWinsController_bwc.on(BigWinsController.EVENT_BIG_WIN_PRESETNTATION_COINS_REQUIRED, this._onSomeBigWinPresentationCoinsRequired, this);
		...
		APP.currentWindow.bossModeController.on(BossModeController.COIN_SPLASH_STARTED, this._onBossCoinsExplodeStart, this);
		APP.currentWindow.bossModeController.on(BossModeController.EVENT_ON_END_SOUND, this._changeBossBgSounds, this);
	...
	_isOrcMusic()
	{
		return this._isActualBgMusic("mq_dragonstone_orc_music");
	}
	...
	_onSecondaryScreenDeactivated()
	{
		...
		if (
			!APP.gameScreen.gameField.isShakeGroundOnFormationOfOrcsActive() &&
			this._isOrcMusic()
			)
		{
			this.__stopSound("mq_dragonstone_orc_music");

			if(!lIsNextMusicRequested_bl)
			{
	...
	{
		this._fFragmentsController_fc = APP.gameScreen.gameField.fragmentsController;
		this._fFragmentsPanelController_fpc = APP.gameScreen.gameField.fragmentsPanelController;

		if (this._fFragmentsPanelController_fpc.isOneFragmentLeftBeforeBossRising)
		{
			this._onOneFragmentLeftBeforeBossRising();
		}
		else
		{
			this._validateMusic();
		...
		this._fFragmentsPanelController_fpc.on(FragmentsPanelController.EVENT_ON_ONE_FRAGMENT_LEFT_BEFORE_BOSS_RISING, this._onOneFragmentLeftBeforeBossRising, this);
		this._fFragmentsController_fc.on(FragmentsController.ON_FRAGMENT_APPEARING_STARTED, this._onFragmentAppearingStarted, this);
	...
	_onNeedMuteBgSoundOnBigWinPlaying(e)
	{
	...
	_onNeedFadeBackBGSoundOnBigWinPlayingCompleted(e)
	{	
		if (!this._fIsGameRoundPlaying_bl)
		{
			return;
	...
	_validateMusic()
	{
		...
		else if (this._fFragmentsPanelController_fpc && this._fFragmentsPanelController_fpc.isOneFragmentLeftBeforeBossRising && !APP.playerController.info.isKicked)
		{
			this._clearMainBgMusicChangeTimer();
			this._playTensionMusic();
		}
	...
	_playMainBgMusic(aOptSmoothly_bl = true)
	{
		...
		let lDoesBossExist_bl = APP.currentWindow.isBossExist;

		if(!this._fFragmentsPanelController_fpc)
		{
			this._fMusicBg_uscc = this.i_getSoundController(lSoundName_str);
			return;
		}
		...
		{
			// lSoundName_str = "mq_dragonstone_orc_music";//TODO: BACK When need sound Orc
		}
		else if(this._isOrcMusic() && !lDoesBossExist_bl)
		{
			this.setVolumeSmoothly("mq_dragonstone_orc_music", 0, CROSS_FADE_DURATION, null);
		}

		if (APP.playerController.info.isKicked)
		{
			lSoundName_str = BG_MAIN_SOUND_NAME + this._fCurrentBgMusicInd_int;
		}

		if (
				lSoundName_str !== "mq_dragonstone_orc_music"
				&& this._fFragmentsPanelController_fpc && this._fFragmentsPanelController_fpc.isOneFragmentLeftBeforeBossRising
				&& !APP.playerController.info.isKicked
			)
		{
			this._clearMainBgMusicChangeTimer();
	...
	_playTensionMusic(aOptSmoothly_bl = false)
	{
		const lSoundName_str = "mq_mus_game_bg_prev_boss";
		...
		if (this._fMusicBg_uscc)
		{
			...
			if (lMusicBgInfo_ussi.i_getSoundName() === lSoundName_str)
			{
				return;
			}
		...
		this._fCurrentBgMusicSound = lSoundName_str;
	}
	 ...
	 _playBgMusic(aSoundName_str, aOptAutoplay_bl = true, aOptSmoothly_bl = false, aOptStopPreviousBgSound_bl = true)
	{
		...
		if(aSoundName_str === "mq_dragonstone_orc_music")
		{
			this._fMusicBg_uscc = this.i_getSoundController(aSoundName_str);
			this._fMusicBg_uscc && this._fMusicBg_uscc.i_startPlaying();
			this._clearMainBgMusicChangeTimer();

			this.setFadeMultiplier(aSoundName_str, 0);
			this.setVolumeSmoothly(aSoundName_str, 1*this._curFadeMult, CROSS_FADE_DURATION);
			return;
		}
		...
		{
			this._fMusicBg_uscc.i_resetVolume();

			if (this._fIsMuteOnBigWinPlayingInProgress_num > 0)
			{
				this._fMusicBg_uscc.i_setVolume(0);
			}
			else
			{
				this._fMusicBg_uscc.i_setVolume(1 * this._curFadeMult * this._fCurrentMusicVolume_num);
			}
	...
	_onBossEnemyRising(aEvent_evnt)
	{
		...
		if (this._fMusicBg_uscc && !this.isLasthandBossView)
		{
			this._fCrossFadeOutBg_uscc = this._fMusicBg_uscc;
			this.setVolumeSmoothly(this._fCurrentBgMusicSound, 0, CROSS_FADE_DURATION, null, () => {
				this._resetCrossFadeOutMusic();
	...
	_onFragmentAppearingStarted()
	{
		this.__playSound("mq_dragonstone_token_appear");
	}

	_onOneFragmentLeftBeforeBossRising()
	{
		if (this._fMusicBg_uscc)
		{
			let lMusicBgInfo_ussi = this._fMusicBg_uscc.i_getInfo();
			if (lMusicBgInfo_ussi.i_getSoundName() === "mq_mus_game_bg_boss")
			{
				return;
			}
		}
		...
		if (this._fMusicBg_uscc)
		{
			this._clearMainBgMusicChangeTimer();
			this._resetCrossFadeOutMusic();
			this._playTensionMusic(true);
		}
	}

	_onBossEnemyDestroying(aEvent_evnt)
	{
		this.__playSound("mq_dragonstone_boss_dragon_death");
	}
	...
	_onGargoyleFlappingWings(aEvent_evnt)
	{
		if(!APP.currentWindow.gameStateController.info.isBossSubround)
		{
			this.__playSound("mq_enemies_gargoyle_fly");
		}
	}

	_onDragonFlappingWings(aEvent_evnt)
	{
		this.__playSound("mq_dragonstone_boss_dragon_move");
	}

	_onDragonFireBreathStarted(aEvent_evnt)
	{
		this.__playSound("mq_dragonstone_boss_dragon_fire");
	}

	_onFragmentLanded(aEvent_evnt)
	{
		if(aEvent_evnt.isFinalFragment)
		{
			this.__playSound("mq_dragonstone_token_collect_trigger");
			
			return;
		}

		this.__playSound("mq_dragonstone_token_collect");
	}

	_onSomeEnemyRageSoundRequired(aEvent_evnt)
	{
		this.__playSound("mq_dragonstone_rage");
	}

	_onMinislotOccured()
	{
		this.__playSound("mq_dragonstone_minislot_trigger");
		this.__playSound("mq_dragonstone_minislot_popup");
	}

	_onMinislotOutro()
	{
		this.__playSound("mq_dragonstone_minislot_outro");
	}

	_onMinislotSpin()
	{
		this.__playSound("mq_dragonstone_minislot_spin");
	}
	...
	_onSomeBigWinPresentationStarted(aEvent_evnt)
	{
		...
		if(aEvent_evnt.isMinislot)
		{
			switch(aEvent_evnt.bigWinTypeId)
			{
				case "MINISLOT_SIMPLE_WIN":
					lSoundName_str = "mq_dragonstone_minislot_win";
					break;
				case "BIG":
					lSoundName_str = "mq_dragonstone_minislot_bigwin";
					break;
				case "HUGE":
					lSoundName_str = "mq_dragonstone_minislot_hugewin";
					break;
				case "MEGA":
					lSoundName_str = "mq_dragonstone_minislot_megawin";
					break;
			}
		}
		else
		...
		this.__playSound(lSoundName_str, false);
		
		let lMusicSoundController_smsc = this.__getMusicSoundController(lSoundName_str);

		if (!lMusicSoundController_smsc)
		{
		...
		let lMusicSoundInfo_smsi = lMusicSoundController_smsc ? lMusicSoundController_smsc.__getInfo() : null;
	...
	_onSomeBigWinPresentationCoinsRequired(aEvent_evnt)
	{
		if(aEvent_evnt.isMinislot)
		{
			this.__playSound("mq_dragonstone_minislot_bigwin_end");
		}
	}
	...
	_onSomeEnemySpawnSoundRequired(aEvent_evnt)
	{
		switch (aEvent_evnt.enemyTypeId)
		{
			...
			{
				this.__playSound("mq_dragonstone_boss_dragon_spawn");
				break;
			}
			case ENEMY_TYPES.RAVEN:
			{
				...
				if (
					this._fLastRavenSoundCallTime_num === undefined ||
					lCurrentTime_num - this._fLastRavenSoundCallTime_num > 2000)
				{
					this._playRavenSpawnSound();
					this._fLastRavenSoundCallTime_num = lCurrentTime_num;
				}
				break;
			}
			case ENEMY_TYPES.CERBERUS:
			{
				this.__playSound("mq_enemies_cerberus_spawn");
				break;
			}
			case ENEMY_TYPES.ORC:
			{
				if(!APP.gameScreen.gameField.isShakeGroundOnFormationOfOrcsActive())
				{
					this.__playSound("mq_enemies_orc_spawn");
				}
				break;
			}
			case ENEMY_TYPES.OGRE:
			{
				this.__playSound("mq_enemies_ogre_spawn");
				break;
			}
		}
	}
	...
	_playRavenSpawnSound()
	{
		if (this._fCurrentRavenCryCount_int != 0) return;

		if(
			this._fLastRavenSoundCallTime_num === undefined ||
			APP.gameScreen.currentTime - this._fLastRavenSoundCallTime_num > 500
			)
		{
			if(this._fRecentRavenSoundCallCount_int < 2)
			{
				const lSoundId_int = APP.isMobile ? 0 : Utils.random(0, 2);
				const lRavenCryCount_int = Utils.random(1, 4);
				const lSoundName_str = "mq_enemies_ravens_spawn_" + lSoundId_int;

				this._startRavenCrows(lSoundName_str, lRavenCryCount_int);
				this._fRecentRavenSoundCallCount_int++;
			}
			else
			{
				this._fRecentRavenSoundCallCount_int = 0;
			...
			this._fLastRavenSoundCallTime_num = APP.gameScreen.currentTime;
			this._fCurrentRavenCryCount_int = 0;
		}
	...
	_startRavenCrows(aSoundName_str, aRavenCryCount_int)
	{
		const lSound_ussc = this.__playSound(aSoundName_str);
		if (lSound_ussc)
		{
			lSound_ussc.once(SimpleSoundController.i_EVENT_SOUND_PLAYING_COMPLETED, () => {
				this._fCurrentRavenCryCount_int++;
				if (this._fCurrentRavenCryCount_int == aRavenCryCount_int)
				{
					this._fCurrentRavenCryCount_int = 0;
				}
				else
				{
					this._startRavenCrows(aSoundName_str, aRavenCryCount_int);
				}
			});
		}
		else
	...
	{
		if (aEvent_evnt.isMasterWin)
		{
			this.__playSound("boss_coins_explode");
		}
	}

	_onOrcsProcessionStarted()
	{
		if(!APP.isSecondaryScreenActive)
		{
			// this._changeMainBgMusic(); //TODO: BACK When need sound Orc
		}
	}

	_onOrcsProcessionFinished()
	{
		if(!APP.isSecondaryScreenActive)
		{
			// this._changeMainBgMusic();//TODO: BACK When need sound Orc
	...
	_playBGScreenSummarySound()
	{
		...
		this.resetFadeMultiplierBgSounds();
	}

	resetFadeMultiplierBgSounds()
	{
		for (let i = 0; i < BG_MAIN_USUAL_SOUNDS.length ; i++)
		{
			this.setFadeMultiplier(BG_MAIN_USUAL_SOUNDS[i], 1*this._curFadeMult);
		}
	}
	...
	destroy()
	{
		...
		if (APP.currentWindow)
		{
			...
			APP.currentWindow.off(GameScreen.EVENT_ON_GARGOYLE_FLAPPING_WINGS, this._onGargoyleFlappingWings, this);
			...
			this._fFragmentsPanelController_fpc && this._fFragmentsPanelController_fpc.off(FragmentsPanelController.EVENT_ON_ONE_FRAGMENT_LEFT_BEFORE_BOSS_RISING, this._onOneFragmentLeftBeforeBossRising, this);
		
			this._fFragmentsController_fc && this._fFragmentsController_fc.off(FragmentsController.ON_FRAGMENT_APPEARING_STARTED, this._onFragmentAppearingStarted, this);
		}
		...
		this._fFragmentsPanelController_fpc = null;
		this._fCurrentRavenCryCount_int = null;
		this._fTicksSinceLastRavenSoundCall_int = null;
