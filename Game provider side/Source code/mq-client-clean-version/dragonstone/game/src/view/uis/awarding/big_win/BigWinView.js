...
import MiniSlotWinAnimation from './MiniSlotWinAnimation';
...
class BigWinView extends SimpleUIView {
	...
	static get EVENT_BIG_WIN_PRESETNTATION_COINS_REQUIRED() { return "EVENT_BIG_WIN_PRESETNTATION_COINS_REQUIRED"; }
	...
	constructor(aParentContainer_sprt, aOptIsMinislotsOrigin_bl)
	{
		super();
		...
		this._fIsMinislotsOrigin_bl = !!aOptIsMinislotsOrigin_bl;
	...
	_startAnimation()
	{
		...
		var lNeedMute_bl = true;


		if (this.uiInfo.isMegaWin)
		{
			this._fBigWinAnimation_bwa = this.addChild(new MegaWinAnimation(lTotalWin_num));
			this.emit(BigWinView.EVENT_BIG_WIN_PRESETNTATION_STARTED, {isMinislot: this._fIsMinislotsOrigin_bl, bigWinTypeId: "MEGA"});
		}
		else if (this.uiInfo.isHugeWin)
		{
			this._fBigWinAnimation_bwa = this.addChild(new HugeWinAnimation(lTotalWin_num));
			this.emit(BigWinView.EVENT_BIG_WIN_PRESETNTATION_STARTED, {isMinislot: this._fIsMinislotsOrigin_bl, bigWinTypeId: "HUGE"});
		}
		else if(this.uiInfo.isBigWin)
		{
			this._fBigWinAnimation_bwa = this.addChild(new BigWinAnimation(lTotalWin_num));
			this.emit(BigWinView.EVENT_BIG_WIN_PRESETNTATION_STARTED, {isMinislot: this._fIsMinislotsOrigin_bl, bigWinTypeId: "BIG"});

			lNeedMute_bl = !this._fIsMinislotsOrigin_bl;
		}
		else
		{
			this._fBigWinAnimation_bwa = this.addChild(new MiniSlotWinAnimation(lTotalWin_num, 1.2*0.7));
			this.emit(BigWinView.EVENT_BIG_WIN_PRESETNTATION_STARTED, {isMinislot: this._fIsMinislotsOrigin_bl, bigWinTypeId: "MINISLOT_SIMPLE_WIN"});

			this._fBigWinAnimation_bwa.position.set(APP.config.size.width/2 + 210, APP.config.size.height/2 + 130);
		...
		!this.uiInfo.isMiniSlotWin && this._fBigWinAnimation_bwa.position.set(APP.config.size.width/2, APP.config.size.height/2);
	...
	_onBigWinCoinsRequired()
	{
		...
		if (
			this.uiInfo.isBigWin ||
			this.uiInfo.isHugeWin ||
			this.uiInfo.isMegaWin
			)
		{
			this.emit(BigWinView.EVENT_BIG_WIN_PRESETNTATION_COINS_REQUIRED, {isMinislot: this._fIsMinislotsOrigin_bl});
		}
	...
	_startFinalCoinsAnimation()
	{
		...
		if (this._fIsMinislotsOrigin_bl && this.uiInfo.isMiniSlotWin && lFinishPos_obj)
		{
			lStartPos_obj = {x: APP.config.size.width/2 + 210, y: APP.config.size.height/2 + 130};
			lFinishPos_obj.x *= 2;
			lFinishPos_obj.y *= 2;
			lOrigPos_obj = {x: APP.config.size.width + (210 * 2), y: APP.config.size.height + (130 * 2)};
		}
		else
		{
			lStartPos_obj = this._screenCenterPoint;
		}
		...
		this._fIsMinislotsOrigin_bl && this.uiInfo.isMiniSlotWin && this._fBigWinCoinsAward_bwca.scale.set(0.51);
	}
