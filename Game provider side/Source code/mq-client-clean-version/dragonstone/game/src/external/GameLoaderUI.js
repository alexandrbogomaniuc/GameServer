...
class LoaderUI extends BaseUI {
	...
	__addBack() {
		...
		// [FIX] Use Graphics instead of missing sprite 'preloader/back'
		let lBack_spr = this.__fPreloaderView_spr.addChild(new PIXI.Graphics());
		lBack_spr.beginFill(0x000000).drawRect(-1920 / 2, -1080 / 2, 1920, 1080); // Assume HD size
	...
	__addLoadingBar() {
		...
		// [FIX] Use Graphics for bar back
		let lBarBack = lBarContainer_spr.addChild(new PIXI.Graphics());
		lBarBack.beginFill(0x666666).drawRect(0, 0, 400, 20); // Placeholder grey bar
		lBarBack.position.set(-200, -10);

		// [FIX] Use Graphics for bar fill
		let lBarFill = lBarContainer_spr.addChild(new PIXI.Graphics());
		lBarFill.beginFill(0xFFFFFF).drawRect(0, 0, 400, 20); // White fill
		lBarFill.position.x = -200 + lFillOffset; // Adjust pos
		lBarFill.position.y = -10;

		var lBarMask = this._barMask = lBarContainer_spr.addChild(new Sprite);
		var lBarMaskGr = lBarMask.addChild(new PIXI.Graphics());
		lBarMaskGr.beginFill(0x00ff00).drawRect(0, 0, 400, 20);

		lBarMask.anchor.set(0, 0.5);
		lBarMask.scale.x = 0.001;
		lBarMask.position.x = -200 + lFillOffset;
		lBarMask.position.y = -10;
