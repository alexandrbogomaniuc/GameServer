...
import { HIT_RESULT_SINGLE_CASH_ID, HIT_RESULT_TREASURE_ID, HIT_RESULT_SPECIAL_WEAPON_ID, HIT_RESULT_ADDITIONAL_CASH_ID, HIT_RESULT_MONEY_WHEEL_WIN_ID } from '../uis/prizes/PrizesController';
...
class ShotResponsesController extends SimpleController {
	...
	_groupResponses(aBaseResponse_sri, aAddResponse_sri)
	{
		...
		//add win
		if (lAddResponse_sri.moneyWheelWin)
		{
			lBaseResponse_sri.moneyWheelWin = +lBaseResponse_sri.moneyWheelWin + lAddResponse_sri.moneyWheelWin;
		}
		...
		//update instanceKill
		lBaseResponse_sri.instanceKill = lBaseResponse_sri.instanceKill || lAddResponse_sri.instanceKill;
						...
						if (lExistingHitResult_obj.id == lAddHitResult_obj.id)
						{
							switch(+lExistingHitResult_obj.id)
							{
								...
								case HIT_RESULT_MONEY_WHEEL_WIN_ID:
									if (seatId != APP.playerController.info.seatId)
									{
										lExistingHitResult_obj.value = Number(lExistingHitResult_obj.value) + Number(lAddHitResult_obj.value);
									}
									break;
