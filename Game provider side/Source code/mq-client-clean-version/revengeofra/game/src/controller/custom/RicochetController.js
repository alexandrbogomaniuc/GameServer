...
class RicochetController extends SimpleController
{
	...
	__initControlLevel()
	{
		...
		APP.gameScreen.gameField.on(GameField.EVENT_ON_RICOCHET_BULLET_DESTROY, this._onBulletDestroy, this);
	...
	_onBulletFlyOut(e)
	{
		...
		bullet.on(RicochetBullet.EVENT_ON_RICOCHET_BULLET_MOVE, this._onBulletMove, this);
		bullet.on(RicochetBullet.EVENT_ON_RICOCHET_BULLET_COLLISION_OCCURRED, this._onBulletCollisionOccurred, this);
	...
	{
		let bullet = e.target;
		this.info.removeBullet(bullet);

		bullet.off(RicochetBullet.EVENT_ON_RICOCHET_BULLET_MOVE, this._onBulletMove, this);
		bullet.off(RicochetBullet.EVENT_ON_RICOCHET_BULLET_COLLISION_OCCURRED, this._onBulletCollisionOccurred, this);

		this._onPlayerRicochetBulletsUpdated();
	}

	_onBulletCollisionOccurred(e)
	{
		let bullet = e.target;
	...
	{
		let bullet = e.target;
		let bulletPos = {x: bullet.x, y: bullet.y};
		...
		for (let point of pointsToCheck)
		{
			...
			if (newPointToMove)
			{
				...
				{
					bullet.destroy(); // Destroy bullet if new direction not needed and it run out of bounds
				}
	...
	_backAmmoForCurrentBullets()
	{
		...
		{
			this.emit(RicochetController.EVENT_ON_RICOCHET_BULLETS_PAYBACK_REQUIRED, {currentBullets: bulletsCount});
		}
	}
	...
	destroy()
	{
		...
		APP.gameScreen.gameField.off(GameField.EVENT_ON_RICOCHET_BULLET_DESTROY, this._onBulletDestroy, this);
