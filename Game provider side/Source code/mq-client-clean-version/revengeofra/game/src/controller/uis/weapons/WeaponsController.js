...
class WeaponsController extends SimpleController
{
	...
	{
		let lInfo_wsi = this.i_getInfo();
		let lId_num = APP.playerController.info.possibleBetLevels.indexOf(APP.playerController.info.betLevel) + 1;
	...
	{
		let lInfo_wsi = this.i_getInfo();
		lInfo_wsi.currentDefaultWeaponId = event.id;
	}
		...
		switch(data.class)
		{
			...
			case SERVER_MESSAGES.SIT_IN_RESPONSE:
				if (data.rid !== -1)
				{
					...
					lInfo_wsi.realAmmo = data.ammoAmount;
					if (this._fPlayerInfo_pi.qualifyWin > 0 && !(APP.currentWindow.gameFrbController.info.frbMode || (data.mode == BonusInfo.TYPE_FRB)))
					{
			...
			case SERVER_MESSAGES.BALANCE_UPDATED:
				...
				if (data.rid !== -1)
				{
					...
					if (clientResultAmmo !== undefined && serverAmmo !== undefined)
					{
						if ((clientResultAmmo + lRicochetBullets_num) != serverAmmo)
						{
							console.error("*** WRONG AMMO*** clientResultAmmo:", clientResultAmmo, "; serverAmmo:", serverAmmo, "; clientAmmo:", clientAmmo, "; clientPendingAmmo:", clientPendingAmmo, 
												"; ricochetBullets:", lRicochetBullets_num);
						}
					}
				}
				//...debug: compare ammo amount on client/server
	...
	_onRemainingShotsUpdated(aEvent_obj)
	{
		...
		{
			console.log(`Reject ammo updating (weaponId: ${aEvent_obj.weaponId}, shots: ${aEvent_obj.shots}) due to QUALIFY state.`);
			return;
		}
	...
	_onRevertAmmoBack(aEvent_obj)
	{
		...
		{
			console.log("Reject ammo reverting due to QUALIFY state.");
			return;
		}
	...
	_decreaseAmmo(aEvent_obj)
	{
		...
		{
			console.log("Reject ammo decreasing due to QUALIFY state.");
			return;
		}
	...
	_proceedAwardedWeapon(aWeapon_obj)
	{
		...
		lInfo_wsi.awardedWeaponId = lWeapon_obj.id;
	...
	{
		let lInfo_wsi = this.i_getInfo();
		lInfo_wsi.awardedWeaponId = null;
	}
