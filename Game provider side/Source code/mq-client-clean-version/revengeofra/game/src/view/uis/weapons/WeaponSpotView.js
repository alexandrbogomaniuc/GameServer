...
class WeaponSpotView extends SimpleUIView
{
	...
	_animateCircleBlast()
	{
		...
		let lScale_seq = [
			{ tweens: [{ prop: "scale.x", to: 2 }, { prop: "scale.y", to: 2 }], duration: 10 * FRAME_RATE, onfinish: () => { this._destroyAnimation(this._fCircleBlast_spr); } },
		];
	...
	_animateSmoke()
	{
		this._fSmoke_spr = this.addChild(APP.library.getSprite("common/transition_smoke_fx_unmult"));
		...
		let lScale_seq = [
			...
			{ tweens: [{ prop: "scale.x", to: 2.3 }, { prop: "scale.y", to: 2.3 }], duration: 13 * FRAME_RATE, onfinish: () => { this._destroyAnimation(this._fSmoke_spr); } },
		];
		Sequence.start(this._fSmoke_spr, lScale_seq);

		let lAlpha_seq = [
			{ tweens: [], duration: 4 * FRAME_RATE },
			{ tweens: [{ prop: "alpha", to: 0 }], duration: 12 * FRAME_RATE },
		];
		Sequence.start(this._fSmoke_spr, lAlpha_seq);

		let lAngle_seq = [
			{tweens: [{prop: 'rotation', to: Utils.gradToRad(196)}],		duration: 12 * FRAME_RATE},
		];
