...
import PreloaderTooltip from '../main/tips/PreloaderTooltip';
...
class LoaderUI extends BaseUI
{
	...
	{
		let lFlareSeq_arr = [
			{tweens: [{prop: 'alpha', to: 0.5}],	duration: 30*FRAME_RATE},
	...
	pauseTooltips()
	{
		...
		if (!this._fTooltips_pt_arr) return;

		this._fTooltips_pt_arr.forEach(tip => tip.id == this._fTipQueue_arr[this._fCurrentTip_num] && tip.pause());
	}
	...
	resumeTooltips()
	{
		...
		this._fTooltips_pt_arr.forEach(tip => tip.id == this._fTipQueue_arr[this._fCurrentTip_num] && tip.resume());
	}
