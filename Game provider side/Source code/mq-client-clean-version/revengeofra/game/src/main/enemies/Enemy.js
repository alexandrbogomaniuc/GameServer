...
import { WEAPONS, ENEMIES, ENEMY_TYPES, ENEMY_BOSS_SKINS, FRAME_RATE, isBossEnemy } from '../../../../shared/src/CommonConstants';
...
class Enemy extends Sprite
{
	...
	get enemyAngle()
	{
		return this._enemyAngle;
	}

	set enemyAngle(val)
	{
		this._enemyAngle = val;
	}
	...
	_startClosePortalTimer()
	{
		...
		if (this._firstPoint.time <= this._gameScreen.currentTime)
		{
	...
	_calculateDirection()
	{
		return Enemy.getDirection(this.enemyAngle);
	}
	...
	_freeze(aIsAnimated_bl = true)
	{
		...
		if (!this._fIsFrozen_bl)
		{
			...
			//don't add freeze effect for Edge and IE and when WebGL is off
	...
	_generateFreezeMask(aResolution_num)
	{
		...
		APP.stage.renderer.render(this.spineView.view, { renderTexture: l_txtr });
	...
	_addFreezeCover()
	{
		...
		let lFreezeMask_sprt = this._fFreezeMask_sprt;
		lFreezeCover_sprt.zIndex = lFreezeMask_sprt.zIndex+1;
		lFreezeCover_sprt.maskSprite = lFreezeMask_sprt;
		lFreezeCover_sprt.pluginName = 'batchMasked';
		//lFreezeMask_sprt.renderable = false;
		lFreezeMask_sprt.alpha = 0.28;
	}
		...
		else
		{
			...
			if (!aIsInstantKill_bl)
			{
				...
				this.spineView.view.state.onComplete = (e) => {
					...
					this._fBonesFellDown_bl = true;
					this.emit(Enemy.EVENT_ON_DEATH_ANIMATION_BONES_FELL_DOWN, {position: lEnemyPosition_pt, angle: this.enemyAngle});
				}
			}
	...
	destroy(purely = false)
	{
		...
		if (!purely)
		{
			...
			lEnemyPosition_pt.x += this.getCurrentFootPointPosition().x;
			lEnemyPosition_pt.y += this.getCurrentFootPointPosition().y;
			this.emit(Enemy.EVENT_ON_ENEMY_VIEW_REMOVING, {position: lEnemyPosition_pt, angle: this.enemyAngle});
		}
