...
import EternalPlazmaSmoke from '../instant_kill/EternalPlazmaSmoke';
...
class Cryogun extends Gun {
	...
	constructor(aIsMaster_bl)
	{
		...
		this._fEternalPlazmaSmoke_eps = null;
	...
	_initReloadState()
	{
		...
		lFrozenCryogun_sprt.fadeTo(1, 16*2*16.7);

		let lFreezeOver_sprt = this._fReloadCover_sprt.addChild(APP.library.getSprite('weapons/Cryogun/cryogun_freeze_over'));
		let lOverMask_sprt = this._fReloadCover_sprt.addChild(APP.library.getSprite('common/rounded_mask'));
		lOverMask_sprt.scale.set(0.6, 0.75);
		lFreezeOver_sprt.mask = lOverMask_sprt;
		lOverMask_sprt.position.set(0, 30);
		lOverMask_sprt.moveTo(0, -30, 29*2*16.7, null, () => {
			lFreezeOver_sprt.mask = null;
			lOverMask_sprt.destroy();
		});
		if(APP.profilingController.info.isVfxProfileValueMediumOrGreater)
		{
			this._fEternalPlazmaSmoke_eps = this._fReloadCover_sprt.addChild(new EternalPlazmaSmoke());
			this._fEternalPlazmaSmoke_eps.position.set(0, -25);
			this._fEternalPlazmaSmoke_eps.scale.set(4*0.15, 4*0.10);
			this._fEternalPlazmaSmoke_eps.scaleTo(4*0.26, 4*0.16, 39*2*16.7);
			this._fEternalPlazmaSmoke_eps.alpha = 0;
			...
			Sequence.start(this._fEternalPlazmaSmoke_eps, lAlphaSequence_seq);
		}
	...
	{
		this._destroyChargeSound();
		Sequence.destroy(Sequence.findByTarget(this._fEternalPlazmaSmoke_eps));
		this._fEternalPlazmaSmoke_eps && this._fEternalPlazmaSmoke_eps.destroy();
		super.destroy();
	}
}
