...
const FLAME_LONG_SEQUENCES = [
								[
									{ tweens: [ { prop: 'scale.x', to: 2 }, { prop: 'scale.y', to: 2 } ], duration: 28*2*16.7 }
								],
								[
									{ tweens: [ { prop: 'rotation', to: Utils.gradToRad(80) } ], duration: 28*2*16.7 }
								],
								[
									{ tweens: [], duration: 19*2*16.7 },
									{ tweens: [ { prop: 'alpha', to: 0 } ], duration: 9*2*16.7 }
								]
							];
...
const FLAMES_DESCR = [
	{// flame layer 19
		...
		sequences: [
						[
							{ tweens: [ { prop: 'scale.x', to: 2 }, { prop: 'scale.y', to: 2 } ], duration: 18*2*16.7 }
						],
						[
	...
	{// flame layer 15
		...
		sequences: [
						[
							{ tweens: [ { prop: 'scale.x', to: 2 }, { prop: 'scale.y', to: 2 } ], duration: 20*2*16.7 }
						],
						[
							{ tweens: [ { prop: 'rotation', to: Utils.gradToRad(80) } ], duration: 20*2*16.7 }
						],
						[
							{ tweens: [], duration: 11*2*16.7 },
							{ tweens: [ { prop: 'alpha', to: 0 } ], duration: 9*2*16.7 }
						]
					]
	},
	...
	{// flame layer 3
		...
		sequences: [
						[
							{ tweens: [ { prop: 'scale.x', to: 1 }, { prop: 'scale.y', to: 1 } ], duration: 17*2*16.7 }
						],
						[
							{ tweens: [ { prop: 'rotation', to: Utils.gradToRad(80) } ], duration: 17*2*16.7 }
						],
						[
...
class FlameThrowerHitFlame extends Sprite 
{
	...
	_addFlame(flameDescription)
	{
		...
		let lFlamePart1_sprt = lBaseFlame_sprt.addChild(APP.library.getSprite('weapons/FlameThrower/hit_flame'));
		lFlamePart1_sprt.alpha = 0.1;
		let lFlamePart2_sprt = lBaseFlame_sprt.addChild(APP.library.getSprite('weapons/FlameThrower/hit_flame'));
