...
class WinBlock extends Sprite
{
	...
	constructor(aCurrencySymbol_str, aIndicatorsUpdateTime_num)
	{
		...
		this._fWinCurrencyView_tf = null;
	...
	_init()
	{
		...
		this._fWinView_tf.position.set(-15, 4);
		...
		this._fWinCurrencyView_tf = this.addChild(new TextField(this._currencyTextFormat));
		this._fWinCurrencyView_tf.anchor.set(0, 0.5);
		this._fWinCurrencyView_tf.position.set(-25.5, 4);

		if (APP.isMobile)
		{
			lWinBase_sprt.position.y = -7.5;
			lWinBase_sprt.scale.set(0.76);

			this._fWinView_tf.position.set(-37, 7);
			this._fWinView_tf.maxWidth = 76;
			this._fWinCurrencyView_tf.position.set(-48, 7);
		}
	...
	_updateCurrency(aCurrencySymbol_str)
	{
		...
		Object.assign(this._fWinCurrencyView_tf.style, {fontFamily: lFontName_str});
		this._fWinCurrencyView_tf.text = this._fCurrencySymbol_str;
	...
	_formatPositions()
	{
		let lCurrencyWidth_num = this._fWinCurrencyView_tf.textBounds.width;

		this._fWinView_tf.position.x = APP.isMobile ? -37 : -15;
		this._fWinView_tf.maxWidth = APP.isMobile ? 76 : 62;

		if (lCurrencyWidth_num > 8)
		{
			let lExtra_num = lCurrencyWidth_num - 8;
			this._fWinView_tf.position.x += lExtra_num;
			this._fWinView_tf.maxWidth -= lExtra_num;
		}
	}
