...
import RoundResultPlayersListView from './RoundResultPlayersListView';
import ProfileAvatar from '../../../ui/profile/ProfileAvatar';
import RoundResultChestView from './RoundResultChestView';
...
class RoundResultScreenView extends SimpleUIView
{
	...
	hideTransparentBack()
	{
		this._fTransparentBack_grphc.visible = false;
	}

	showTransparentBack()
	{
		this._fTransparentBack_grphc.visible = true;
	}
	...
	constructor()
	{
		...
		this._fTransparentBack_grphc = null;
		...
		this._fMoneyWheelPayoutsIndicatorView_rrbpiv = null;
		this._fMoneyWheelPayoutsCounter_c = null;
	...
	_addTransparentBack()
	{
		this._fTransparentBack_grphc = this.addChild(new PIXI.Graphics());

		this._fTransparentBack_grphc.beginFill(0x000000, 0.45).drawRect(-480, -284, 960, 560).endFill();
		this._fTransparentBack_grphc.interactive = true;
		this._fTransparentBack_grphc.buttonMode = false;
	}

	_addBack()
	{
		let lBack_spr = this.addChild(APP.library.getSprite("round_result/back"));

		let lIcon_spr = this.addChild(APP.library.getSprite("round_result/icon"));
		lIcon_spr.position.set(-291, -172.5);
	...
	_addBackLines()
	{
		this._addSpriteLine(-160.5, -50, 0.44, Math.PI / 2);
		this._addSpriteLine(-154.5, 19.5);

		this._addGraphicLine(-100.5, 51.5);
		this._addGraphicLine(-210.5, 51.5);

		this._addSpriteLine(-154.5, 81.5);
		...
		let lFrame_sprt = this.addChild(APP.library.getSprite('round_result/frame'));
		lFrame_sprt.position.set(149, 126);
	}
	...
	_addSpriteLine(aX, aY, aScale = 1, aRotation = 0)
	{
		let lLine_spr = this.addChild(APP.library.getSprite("round_result/line"));
		lLine_spr.position.set(aX, aY);
		lLine_spr.scale.x = aScale;
		lLine_spr.rotation = aRotation;
	}
	...
	_addCaptions()
	{
		...
		lTotalDamageCaption_cta.position.set(-88, -36.5);
		...
		lTotalKillsCaption_cta.position.set(-154, 34.5);
		...
		this._fMoneyWheelWinsCaption_cta = this.addChild(I18.generateNewCTranslatableAsset("TARoundResultMoneyWheelWinsCaption"));
		this._fMoneyWheelWinsCaption_cta.position.set(-264, 100);

		this._fMoneyWheelPayoutsCaption_cta = this.addChild(I18.generateNewCTranslatableAsset("TARoundResultMoneyWheelPayoutsCaption"));
		this._fMoneyWheelPayoutsCaption_cta.position.set(-154, 100);
		...
		{
			this._fMoneyWheelWinsCaption_cta.position.set(-209, 100);
			this._fMoneyWheelPayoutsCaption_cta.position.set(-99, 100);
		}
	}
	...
	_addGraphics()
	{
		this._fChest_rrcv = this.addChild(new RoundResultChestView());
		this._fChest_rrcv.position.set(-86.5, -99);
	...
	_initAvatar()
	{
		...
		if (lAvatarData_obj)
		{
			...
			//this._fAvatarView_pa.scale.set(0.56);
	...
	_addPlayersList()
	{
		this._fPlayersListView_rrplv = this.addChild(new RoundResultPlayersListView());
		this._fPlayersListView_rrplv.position.set(149, -120);
	}
	...
	_addValues()
	{
		...
		this._fTotalDamageValue_rrtiv.position.set(-91, -17);
		...
		this._fMoneyWheelWinsCountValue_rrtiv = this.addChild(new RoundResultTextIndicatorView(new TextField(this._partialValuesStyle), false, 0.5, 0.5));
		this._fMoneyWheelWinsCountValue_rrtiv.maxWidth = 40;
		this._fMoneyWheelWinsCountValue_rrtiv.position.set(-263, 121.5);
		this._fMoneyWheelWinsCountCounter_c = new Counter({ target: this._fMoneyWheelWinsCountValue_rrtiv, method: "value" });

		this._fMoneyWheelPayoutsCounter_c = new Counter({ target: this.moneyWheelPayoutsIndicatorView, method: "indicatorValue" });
		...
		{
			this.moneyWheelPayoutsIndicatorView.position.set(-103, 121.5);
			this._fMoneyWheelWinsCountValue_rrtiv.position.set(-208, 121.5);
		}
	}

	get moneyWheelPayoutsIndicatorView()
	{
		return this._fMoneyWheelPayoutsIndicatorView_rrbpiv || (this._fMoneyWheelPayoutsIndicatorView_rrbpiv = this._initMoneyWheelPayoutsIndicatorView());
	}

	_initMoneyWheelPayoutsIndicatorView()
	{
		let l_rrqpiv = this.addChild(new RoundResultPayoutsIndicatorView());
		l_rrqpiv.position.set(-158, 121.5);
		return l_rrqpiv;
	}
	...
	{
		let l_rrqpiv = this.addChild(new RoundResultPayoutsIndicatorView());
		l_rrqpiv.position.set(-52, 121.5);
		return l_rrqpiv;
	}
	...
	_updateValues(aSkipAnimation_bl = false)
	{
		...
		if (l_rrsi)
		{
			...
			if (aSkipAnimation_bl)
			{
				...
				this._fMoneyWheelWinsCountCounter_c && this._fMoneyWheelWinsCountCounter_c.stopCounting();
				this._fMoneyWheelWinsCountValue_rrtiv.value = l_rrsi.moneyWheelWinsCount;

				this._fMoneyWheelPayoutsCounter_c && this._fMoneyWheelPayoutsCounter_c.stopCounting();
				this.moneyWheelPayoutsIndicatorView.indicatorValue = l_rrsi.moneyWheelPayouts;
				...
				this._fChest_rrcv.show();
				this._fChest_rrcv.startAnimation();
			...
			else
			{
				...
				this._fMoneyWheelWinsCountValue_rrtiv.value = 0;
				this.moneyWheelPayoutsIndicatorView.indicatorValue = 0;
				if (this._compensationNeeded)
				{
	...
	_startCountingBlock5()
	{
		...
		this._fMoneyWheelWinsCountCounter_c.startCounting(
			this.uiInfo.moneyWheelWinsCount,
			COUNTING_DURATION_PARTIAL,
			null,
			this._startCountingBlock6.bind(this)
		);

		if (this.uiInfo.moneyWheelWinsCount != 0)
		{
			this.emit(RoundResultScreenView.ON_COINS_ANIMATION_STARTED);
			this._fCoinsAnimationDelay_t = new Timer(this._startCoinsAnimation.bind(this, this._fMoneyWheelWinsCountValue_rrtiv), COINS_TIME_RANGE, true);
		}
	}

	_startCountingBlock6()
	{
		this._resetCoinDelay();
		this.emit(RoundResultScreenView.ON_COINS_ANIMATION_COMPLETED);

		this._fMoneyWheelPayoutsCounter_c.startCounting(
			this.uiInfo.moneyWheelPayouts,
			COUNTING_DURATION_PARTIAL,
			null,
			this._startCountingBlock7.bind(this)
		);

		if(this.uiInfo.totalDamage != 0)
		{
			if (this.uiInfo.moneyWheelPayouts != 0)
			{
				this.emit(RoundResultScreenView.ON_COINS_ANIMATION_STARTED);
				this._fCoinsAnimationDelay_t = new Timer(this._startCoinsAnimation.bind(this, this._fMoneyWheelPayoutsIndicatorView_rrbpiv), COINS_TIME_RANGE, true);
				
			}
			this._onePulseBeatAnimation(this._fTotalDamageValue_rrtiv);
		}
	}

	_startCountingBlock7()
	{
		if (!this._compensationNeeded)
		{
			return;
	...
	_stopAllAnimation()
	{
		this._fChest_rrcv.cleareAnimation();
		this._fTotalDamageCounter_c && this._fTotalDamageCounter_c.stopCounting();
		this._fBulletsFiredCounter_c && this._fBulletsFiredCounter_c.stopCounting();
		this._fMoneyWheelWinsCountCounter_c && this._fMoneyWheelWinsCountCounter_c.stopCounting();
		this._fMoneyWheelPayoutsCounter_c && this._fMoneyWheelPayoutsCounter_c.stopCounting();
	...
	_showNextButton()
	{
		...
		this._showWaitingCaption_bl = false;

		if (!this._nextRoundBtn)
		{
			let lNextRoundButton_btn = this._nextRoundBtn = this.addChild(new RoundResultButton("round_result/active_btn", "round_result/deactive_btn", "TARoundResultNextRoundBtnCaption", false, undefined, RoundResultButton.BUTTON_TYPE_ACCEPT));
			lNextRoundButton_btn.position.set(145.5, 126);
			lNextRoundButton_btn.on("pointerclick", this._onNextRoundButtonClicked, this);
		}
	...
	_invalidateNextButton()
	{
		if (this._nextRoundBtn)
		{
			...
			this._nextRoundBtn.visible = this._showNextButton_bl && lGameStateInfo_gsi.isPlayerSitIn && !lIsCompletedBonusesExist_bl && !lIsFrbEnded_bl;;
		}
	}
	...
	_hideNextButton()
	{
		if (this._nextRoundBtn)
		{
			this._nextRoundBtn.visible = false;
		}
	}
	...
	_validateCaptions()
	{
		if (this._compensationNeeded)
		{
			...
			this._fMoneyWheelWinsCaption_cta && this._fMoneyWheelWinsCaption_cta.position.set(-264, 100);
			this._fMoneyWheelPayoutsCaption_cta && this._fMoneyWheelPayoutsCaption_cta.position.set(-154, 100);

			this.moneyWheelPayoutsIndicatorView.position.set(-158, 121.5);
			this._fMoneyWheelWinsCountValue_rrtiv.position.set(-263, 121.5);
		}
		else
		{
			...
			this._fMoneyWheelWinsCaption_cta && this._fMoneyWheelWinsCaption_cta.position.set(-209, 100);
			this._fMoneyWheelPayoutsCaption_cta && this._fMoneyWheelPayoutsCaption_cta.position.set(-99, 100);
	...
	destroy()
	{
		...
		this._fMoneyWheelWinsCountValue_rrtiv && this._fMoneyWheelWinsCountValue_rrtiv.destroy();
		this._fMoneyWheelWinsCountValue_rrtiv = null;
		this._fMoneyWheelWinsCountCounter_c && this._fMoneyWheelWinsCountCounter_c.destroy();
		this._fMoneyWheelWinsCountCounter_c = null;

		this._fMoneyWheelPayoutsIndicatorView_rrbpiv && this._fMoneyWheelPayoutsIndicatorView_rrbpiv.destroy();
		this._fMoneyWheelPayoutsIndicatorView_rrbpiv = null;
		this._fMoneyWheelPayoutsCounter_c && this._fMoneyWheelPayoutsCounter_c.destroy();
		this._fMoneyWheelPayoutsCounter_c = null;
