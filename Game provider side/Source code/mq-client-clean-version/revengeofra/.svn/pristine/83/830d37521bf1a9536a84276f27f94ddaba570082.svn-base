...
class TargetingController extends SimpleUIController {
	...
	__init()
	{
		...
		lAutoTargetingSwitcher_atsc.on(AutoTargetingSwitcherController.EVENT_ON_AUTO_TARGETING_SWITCHED, this._onAutoTargetinSwitched, this);
	}
	...
	{
		if (this.info.isActive && this._fWeaponsInfo_wsi.currentWeaponId === WEAPONS.ARTILLERYSTRIKE && this.info.targetEnemyId === event.enemyId)
		{
			this._tryToSelectNextTarget();
		}
	}

	_onAutoTargetinSwitched(event)
	{
	...
	_updateTargeting(aTargetEnemyId_int)
	{
		...
		if (!!lTargetEnemy_enm)
		{
			this._fCurTargetPreference_int = lTargetEnemy_enm.energy;
		}
	...
	_onFireSettingsScreenActivated(event)
	{
		this._fIsFireSettingsActivated_bl = true;
		this._validatePause();
	}

	_onFireSettingsScreenDeactivated(event)
	{
		this._fIsFireSettingsActivated_bl = false;
		this._validatePause();
	}
	...
	{
		let lCurTargetPriorityType_int = APP.playerController.info.targetPriority;
		let lCurTargetPreference_int = this._fCurTargetPreference_int;
		let lEnemies_obj_arr = this._gameScreen.getEnemies().slice();
		lEnemies_obj_arr.sort(
								function (a, b)
								{
									let lEnemyAPreference_int = a.energy;
									let lEnemyBPreference_int = b.energy;
									if (
											lCurTargetPriorityType_int == 2
											&& lCurTargetPreference_int !== undefined
										) // SAME
									{
		...
		lEnemies_obj_arr = lEnemies_obj_arr.filter(function(value, index, self) {
			...
			let lEnemyView_enm = lGameScreen_gs.gameField.getExistEnemy(lEnemyId_int);
			return (value.life !== 0
				&& !value.killedMiss
				&& lEnemyView_enm
