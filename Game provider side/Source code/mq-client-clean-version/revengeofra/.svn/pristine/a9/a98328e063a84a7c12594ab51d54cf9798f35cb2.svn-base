...
class BalanceBlock extends Sprite
{
	...
	updateCurrency(aCurrencySymbol_str)
	{
		this._updateCurrency(aCurrencySymbol_str);
	}
	...
	constructor(aCurrencySymbol_str, aIndicatorsUpdateTime_num)
	{
		super();
		...
		this._fBalanceCurrencyView_tf = null;
	...
	_init()
	{
		...
		this._fBalanceView_tf.position.set(20, 4);
		...
		this._fBalanceCurrencyView_tf = this.addChild(new TextField(this._currencyTextFormat));
		this._fBalanceCurrencyView_tf.anchor.set(0, 0.5);
		this._fBalanceCurrencyView_tf.position.set(10.5, 4);
		...
		if (APP.isMobile)
		{
			...
			this._fBalanceView_tf.position.set(-3, 7);
			this._fBalanceView_tf.maxWidth = 76;
			this._fBalanceCurrencyView_tf.position.set(-15, 7);
		}

		this._updateCurrency(this._fCurrencySymbol_str);
	...
	_updateCurrency(aCurrencySymbol_str)
	{
		this._fCurrencySymbol_str = aCurrencySymbol_str;

		let lIsFontIncludesCurrencySymbol_bl = APP.fonts.isGlyphsSupported(this._currencyTextFormat.fontFamily, this._fCurrencySymbol_str);
		let lFontName_str = lIsFontIncludesCurrencySymbol_bl ? this._currencyTextFormat.fontFamily : "fnt_nm_arial_currency";
		Object.assign(this._fBalanceCurrencyView_tf.style, {fontFamily: lFontName_str});
		this._fBalanceCurrencyView_tf.text = this._fCurrencySymbol_str;
	...
	_formatPositions()
	{
		let lCurrencyWidth_num = this._fBalanceCurrencyView_tf.textBounds.width;

		this._fBalanceView_tf.position.x = APP.isMobile ? -3 : 20;
		this._fBalanceView_tf.maxWidth = APP.isMobile ? 76 : 62;

		if (lCurrencyWidth_num > 8)
		{
			let lExtra_num = lCurrencyWidth_num - 8;
			this._fBalanceView_tf.position.x += lExtra_num;
			this._fBalanceView_tf.maxWidth -= lExtra_num;
		}
	}
