...
const MISSILES_INTERVAL = 7*2*16.7;
...
class ArtilleryStrikeView extends SimpleUIView {
	...
	get _mainContainer ()
	{
		return this._gameScreen.gameField.artilleryStrikeEffectContainer.container;
	}
	...
	{		
		...
		this._fArtilleryGrenade_sprt = new ArtilleryGrenade(bulletProps, points, _ => {
												this._onArtilleryGrenadeLanded();
											}, this.uiInfo.rid);

		this._mainContainer.addChild(this._fArtilleryGrenade_sprt);
	...
	_startSmoke()
	{
		this._fArtilleryGrenadeSmokes_agss = new ArtilleryGrenadeSmokes();
		this._mainContainer.addChild(this._fArtilleryGrenadeSmokes_agss);
		...
		this._fStrikesStartTimer_tmr = new Timer(this._onTimeToStartArtilleryStrikes.bind(this), 47*2*16.7);
	...
	{		
		for (let i=0; i<aTargetEnemiesIds_int_arr.length; i++)
		{
			...
			let lInterval_num = MISSILES_INTERVAL * i;
	...
	_launchAnotherArtilleryMissile(aTargetEnemyId_int)
	{
		...
		let startPos = new PIXI.Point(endPos.x, -10);
		...
		this._mainContainer.addChild(lArtilleryMissile_am);
		lArtilleryMissile_am.zIndex = endPos.y;
		lArtilleryMissile_am.rotation = Math.PI;

		lArtilleryMissile_am.position.set(startPos.x, startPos.y);
		this._fArtilleryMissiles_am_arr.push(lArtilleryMissile_am);
		
		lArtilleryMissile_am.moveTo(endPos.x, endPos.y, time, Easing.quadratic.easeIn, _=> {
	...
	_fadeOutGrenade()
	{
		this._fArtilleryGrenadeSmokes_agss && this._fArtilleryGrenadeSmokes_agss.fadeTo(0, 13*2*16.7);
		this._fArtilleryGrenade_sprt.fadeTo(0, 13*2*16.7, null, _=> {
	...
	{
		if 	( 
				(!this._fArtilleryMissiles_am_arr || this._fArtilleryMissiles_am_arr.length === 0)
			 	&& !this._fArtilleryGrenade_sprt
			) 
		{
