...
class SpineEnemy extends Enemy
{
	...
	_updateTint()
	{
		...
		{
			this.spineView && this.spineView.tintIt(this.tintColor);
		}
		else
		{
			this._untint();
	...
	changeSpineView(type, noChangeFrame)
	{
		...
		if (type == STATE_DEATH)
		{
			...
			lNewSpineName_str = this._getDeathSpineName();

			if (this.isCritter || this.isBoss || !lNewSpineName_str)
			{
				this.destroy();
				return;
			}
	...
	setDeathFramesAnimation(aIsInstantKill_bl = false, aPlayerWin_obj = null)
	{
		...
		lEnemyPosition_pt.x += this.getCurrentFootPointPosition().x;
		lEnemyPosition_pt.y += this.getCurrentFootPointPosition().y;
		this.emit(Enemy.EVENT_ON_DEATH_ANIMATION_STARTED, {position: lEnemyPosition_pt, angle: this.enemyAngle});
	}
