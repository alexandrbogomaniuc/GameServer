...
const TREASURES =
{
	WHEEL_KEY: 	{name: "WHEEL_KEY", key_id: 1}
}
class PrizesController extends SimpleController {
	...
	_onTimeToShowPrizes(event)
	{
		...
		if (!isNaN(hitData.moneyWheelWin) && hitData.moneyWheelWin > 0)
		{
		...
		for (let lSeatId_int in hitData.hitResultBySeats)
		{
					...
					case HIT_RESULT_ADDITIONAL_CASH_ID:
						...
						if (lWinCash_num > 0)
						{
							...
							let lCashWinParams_obj = {
														...
														isBoss: isBoss,
														isBossMasterFinalWin: (lIsMasterSeat_bl && lPrize_obj.id === HIT_RESULT_ADDITIONAL_CASH_ID)
													};
							if (lIsMasterSeat_bl)
							{
			...
			//TREASURES...
			if (!lIsMasterSeat_bl && !!lTreasureTypes_str_arr.length)
			{
				let lTreasureIds_int_arr = [];

				for (let i=0; i<lTreasureTypes_str_arr.length; i++)
				{
					let lTreasureId_str = lTreasureTypes_str_arr[i];
					for (let lChest_key in TREASURES)
					{
						let lChest_obj = TREASURES[lChest_key];
						if (lChest_obj.name === lTreasureId_str)
						{
							let lTreasureId_int = lChest_obj.key_id;
							if (isNaN(lTreasureId_int))
							{
								throw new Error('wrong treasure name: ' + lTreasureId_str);
							}

							lTreasureIds_int_arr.push(lTreasureId_int);
						}
					}
				}

				this.emit(PrizesController.i_EVENT_ON_TIME_TO_SHOW_TREASURE_PRIZE, {treasureIds: lTreasureIds_int_arr, seatId: +lSeatId_int, enemyId: enemyId});
			}
			//...TREASURES
		...
		if (lMoneyWheelValue_num > 0)
		{
			this.emit(PrizesController.i_EVENT_ON_TIME_TO_SHOW_MONEY_WHEEL_PRIZE, {hitData: hitData, value: lMoneyWheelValue_num});
		}
