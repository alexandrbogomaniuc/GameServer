...
class EternalPlazmaSmoke extends Sprite {
	constructor(indigoSmoke = false, indigoBlendMode = "normal"){
		super();

		let assetName = indigoSmoke ? 'weapons/InstantKill/plasmasmoke_indigo' : 'weapons/InstantKill/plasmasmoke';
		let texture = APP.library.getSprite(assetName).textures[0];
