...
const TRANSITION_SMOKES_DESCR = [
	{// TransitionSmoke layer 40
		...
		moveDelta: {position: new PIXI.Point(0, -35), duration: 16*2*16.7},
		...
		sequences: [
						[
							{ tweens: [ { prop: 'scale.x', to: 4*0.4 }, { prop: 'scale.y', to: 4*0.4 } ], duration: 40*2*16.7 }
						],
						[
							{ tweens: [ { prop: 'rotation', to: Utils.gradToRad(-132) } ], duration: 40*2*16.7 }
						],
						[
							{ tweens: [ { prop: 'alpha', to: 0.3*3 } ], duration: 18*2*16.7 },
							{ tweens: [ { prop: 'alpha', to: 0 } ], duration: 22*2*16.7 }
						]
					]
	},
	{// TransitionSmoke layer 23
		position: new PIXI.Point(FX_BASE_POSITION.x+9, FX_BASE_POSITION.y-66),
		moveDelta: {position: new PIXI.Point(-1, -35), duration: 16*2*16.7},
		...
		sequences: [
						[
							{ tweens: [ { prop: 'scale.x', to: 4*0.4 }, { prop: 'scale.y', to: 4*0.4 } ], duration: 15*2*16.7 }
						],
						[
							{ tweens: [ { prop: 'rotation', to: Utils.gradToRad(-102) } ], duration: 16*2*16.7 }
						],
						[
							{ tweens: [ { prop: 'alpha', to: 0.1*3 } ], duration: 6*2*16.7 },
	...
	{// TransitionSmoke layer 22
		...
		moveDelta: {position: new PIXI.Point(-1.5, -48), duration: 22*2*16.7},
		...
		sequences: [
						[
							{ tweens: [ { prop: 'scale.x', to: 4*0.4 }, { prop: 'scale.y', to: 4*0.4 } ], duration: 21*2*16.7 }
						],
						[
							{ tweens: [ { prop: 'rotation', to: Utils.gradToRad(-114) } ], duration: 22*2*16.7 }
						],
						[
							{ tweens: [], duration: 5*2*16.7 },
							{ tweens: [ { prop: 'alpha', to: 0.1*3 } ], duration: 7*2*16.7 },
							{ tweens: [ { prop: 'alpha', to: 0 } ], duration: 10*2*16.7 }
						]
					]
	},
	{// TransitionSmoke layer 21
		...
		sequences: [
						...
						[
							{ tweens: [ { prop: 'rotation', to: Utils.gradToRad(-123) } ], duration: 22*2*16.7 }
						],
						[
							{ tweens: [], duration: 5*2*16.7 },
							{ tweens: [ { prop: 'alpha', to: 0.1*3 } ], duration: 7*2*16.7 },
	...
	{// TransitionSmoke layer 20
		...
		sequences: [
						[
							{ tweens: [ { prop: 'scale.x', to: 4*0.6 }, { prop: 'scale.y', to: 4*0.6 } ], duration: 6*2*16.7 }
						],
						[
							{ tweens: [ { prop: 'rotation', to: Utils.gradToRad(-92) } ], duration: 6*2*16.7 }
						],
						[
							{ tweens: [ { prop: 'alpha', to: 1 } ], duration: 6*2*16.7 },
							{ tweens: [ { prop: 'alpha', to: 0 } ], duration: 10*2*16.7 }
						]
					]
	}
...
class FlameThrowerHitFXs extends Sprite 
{
	...
	_addTorch(aPosition_pt, aRotationInGrad_num)
	{
		...
		let seq = [
						{ tweens: [ { prop: 'alpha', to: 1 } ], duration: 4*2*16.7 },
						{ tweens: [], duration: 22*2*16.7 },
						{ 
