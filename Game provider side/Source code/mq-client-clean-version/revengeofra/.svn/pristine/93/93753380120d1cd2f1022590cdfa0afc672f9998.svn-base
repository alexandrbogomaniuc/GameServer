...
class ContentItem extends Sprite
{
	...
	_addPrizeView()
	{
		...
		if (assetName !== undefined)
		{
			...
			if (itemInfo.isWeaponType || APP.profilingController.info.isVfxProfileValueMediumOrGreater) //always for weapon type; for others depends on VFX
			{
	...
	{
		if (!this.info.awardedWeapon) //it means this is co-player's weapon
		{
			return;
		}
		...
		lFreeShotsCounterView_fscv.i_startFlying(new PIXI.Point(initialX + topXDistance, initialY + topYDistance + AMMO_COUNTER_OFFSET));;
	...
	_animate()
	{
		...
		let lAwardSound_str = "quests_treasure_drop";
	...
	_animateKeySequence(aContent_sprt, x, y)
	{
		...
		{
			let lFlare_sprt = this._fFlare_sprt = aContent_sprt.addChild(APP.library.getSprite("round_result/chest/flare"));
