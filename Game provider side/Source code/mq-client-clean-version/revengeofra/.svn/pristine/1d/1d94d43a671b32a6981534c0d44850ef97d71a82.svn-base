...
class GlowingSprite extends Sprite {
	...
	_createDuplicateSprite()
	{
		...
		this.baseSprite.filters = [new GlowFilter({distance: this._distance, outerStrength: this._outerStrength, innerStrength: this._innerStrength, color: this._glowColor, quality: this._quality, padding: 4})]; // + padding: 4, Not sure

		var l_txtr = PIXI.RenderTexture.create({ width: lBaseLocBounds_obj.width*this.baseSprite.scale.x, height: lBaseLocBounds_obj.height*this.baseSprite.scale.y, scaleMode: PIXI.SCALE_MODES.LINEAR, resolution: 2});
