...
class GameProfilingInfo extends ProfilingInfo {
	...
	_reduceVFXProfileValueIfRequired(aActualProfileValue_str)
	{
		...
		else if (lastFPS < FPS_HIGH_PROFILE_TRESHOLD)
		{
			...
			let curExplosionsAmount = lGameOptimizationinfo_goi.currentMineExplosionsNumber
										+ lGameOptimizationinfo_goi.currentGrenadeExplosionsNumber
			...
			let curTreasureAwardsAmount = lGameOptimizationinfo_goi.currentTreasuresAnimationsAmount + lGameOptimizationinfo_goi.currentCurrentMoneyWheelAmount;
