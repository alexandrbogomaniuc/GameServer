...
import PlayerCollectionScreenController from './player_collection/PlayerCollectionScreenController';
...
class SecondaryScreenController extends SimpleUIController
{
	...
	initView()
	{
		this._initView(new SecondaryScreenView(this._fViewParentContainer_sprt));
	}
	...
	get playerCollectionScreenController()
	{
		return this._playerCollectionScreenController;
	}
	...
	constructor(...args)
	{
		...
		this._fPlayerCollectionScreenController_assc = null;
	...
	__initControlLevel()
	{
		...
		this._playerCollectionScreenController.init();
	...
	__initViewLevel()
	{
		...
		this._playerCollectionScreenController.initView(lView_ssv.playerCollectionScreenView);
		this._playerCollectionScreenController.hideScreen();
	...
	_addEventListeners()
	{
		...
		APP.on(LobbyAPP.EVENT_ON_PLAYER_COLLECTION_BUTTON_CLICKED, this._onPlayerCollectionButtonClicked, this);
		...
		APP.on(LobbyAPP.EVENT_LOBBY_ROOM_WEAPONS_INDICATOR_CLICK, this._onWeaponsIndicatorClicked, this);
		APP.on(LobbyAPP.EVENT_ON_SOME_BONUS_STATE_CHANGED, this._onSomeBonusStateChanged, this);

		this._playerCollectionScreenController.on(PlayerCollectionScreenController.EVENT_ON_CLOSE_BTN_CLICKED, this._onCloseButtonClicked, this);
	...
	_onPlayerCollectionButtonClicked(event)
	{
		this._showScreen(this._playerCollectionScreenController);
	}
	...
	_onWeaponsIndicatorClicked(event)
	{

		this._playerCollectionScreenController.configureWeaponsScreenMode(event.stake);
		this._showScreen(this._playerCollectionScreenController);
	}
	...
	_getCurrentScreenId()
	{
		...
		else if (this._fCurrentScreenController_ssc instanceof PlayerCollectionScreenController) lId_str = LobbyStateInfo.SCREEN_PLAYER_COLLECTIONS;
	...
	{
		return this._fPlayerCollectionScreenController_assc || (this._fPlayerCollectionScreenController_assc = new PlayerCollectionScreenController());
	}
