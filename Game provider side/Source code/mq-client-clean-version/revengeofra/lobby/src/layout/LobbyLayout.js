...
class LobbyLayout extends DOMLayout 
{
	...
	addGame(gameUrl) 
	{
		...
		{
			lGame_gl = GameLayer.create(wrapper, gameUrl, this._isIPhone);
	...
	_resizeLayers(appRect)
	{
		...
		let lIOS13Scale_num = this._getIOSScale();
		...
		if (node)
		{
			...
			let bottomMargin = isNaN(this.margin) ? this.margin[2] || 0 : +this.margin;
			let commonPanelHeight = Math.floor((commonPanelBaseHeight-bottomMargin)*this.scale);
			commonPanelTop = commonPanelTop + appRect.height - (commonPanelHeight * lIOS13Scale_num);

			Object.assign(node.style, {
				left: appRect.left + "px",
				top: commonPanelTop + "px",
				width: appRect.width + "px",
				height: Math.floor(commonPanelBaseHeight*this.scale * lIOS13Scale_num) + "px"
			});
		...
		if (this._isIPhone && this._fGames_arr)
		{
			if (this._fGames_arr.length > 0)
			{
				for (let i = 0; i < this._fGames_arr.length; ++i)
				{
					let lGameFrame_gf = this._fGames_arr[i].gameFrame;
					if (lGameFrame_gf)
					{
						Object.assign(lGameFrame_gf.style, {
							left: appRect.left + "px",
							top: appRect.top + "px",
							width: appRect.width + "px",
		...
		if(this._isIPhone && lIOS13Scale_num < 1 && this._fGames_arr)
		{
			for (let i = 0; i < this._fGames_arr.length; ++i)
			{
				let lGameFrame_gf = this._fGames_arr[i].gameFrame;
				...
				{
					Object.assign(lGameFrame_gf.style, {
						left: (appRect.left - ((1 - lIOS13Scale_num) * Math.floor(this.size.width * this.scale)) / 2) + "px",
						top: (appRect.top - ((1 - lIOS13Scale_num) * Math.floor(this.size.height * this.scale)) / 2) + "px",
						width: Math.floor(this.size.width * this.scale) + "px",
						height: Math.floor(this.size.height * this.scale) + "px"
					});
				}
			}
		}
	...
	_getIOSScale()
	{
		...
		if(this._isIOS && this._is13VersionOfIOS)
		{
			lScale_num = 0.9;
		}
	...
	get _is13VersionOfIOS()
	{
		let useragent = navigator.userAgent.toLowerCase();
		let lIs13_bl = false;

		if (useragent.indexOf('version/13') > -1)
		{
			lIs13_bl = true;
		}
