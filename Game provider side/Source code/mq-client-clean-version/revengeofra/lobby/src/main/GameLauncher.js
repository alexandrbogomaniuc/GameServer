...
class GameLauncher extends EventDispatcher
{
	...
	{
		this.addGame(event.startGameUrl);
	}
	...
	onGameMessageReceived(event)
	{
		...
		switch (msgType)
		{
			...
			case GAME_MESSAGES.GAME_LOADING_ERROR:
				this._app.lobbyScreen.visible = false;
				this._app.lobbyScreen.hideLobby();
			...
			case GAME_MESSAGES.ON_SIT_IN_RESPONSE:
				if (msgData.rid > -1)
				{
					this._app.commonPanelController.onGameSitStateChanged(true);
				}
				break;

			case GAME_MESSAGES.ON_SIT_OUT_REQUIRED:
				this._app.commonPanelController.onGameSitStateChanged(false);
				break;

			case GAME_MESSAGES.REFRESH_COMMON_PANEL_REQUIRED:
				this._app.commonPanelController.refreshUI(event.data);
				break;
		}
	}
