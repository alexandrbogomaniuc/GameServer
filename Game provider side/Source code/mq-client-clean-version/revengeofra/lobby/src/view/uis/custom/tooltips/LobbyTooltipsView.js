...
class LobbyTooltipsView extends SimpleUIView
{
	...
	_init()
	{
		this._addTooltip({x: -1, y: (APP.isMobile ? -120 : -118)},	"TATipsSelectRoom",	null,	LobbyTooltipsView.ID_SELECT_ROOM);
	}

	_addTooltip(aPos_obj, aTAsset_str, aArrowDir_str, aId_num, aOptDurationMs_num = LobbyTooltipsView.DEFAULT_TIP_DURATION)
	{
		let lTooltip_tv = new TooltipView(aTAsset_str, aId_num, aArrowDir_str, aOptDurationMs_num);
	...
	_onTooltipShown(aEvent_obj)
	{
		...
		this.emit(LobbyTooltipsView.EVENT_ON_TIP_SHOWN, {id: lTipId_num});
	}
	...
	_onTooltipHided(aEvent_obj)
	{
		...
		this.emit(LobbyTooltipsView.EVENT_ON_TIP_HIDED, {id: lTipId_num});
	...
	_tryNextTooltip()
	{
		...
		{
			this._fCurrentTooltipId_num = null;
			this.emit(LobbyTooltipsView.EVENT_ON_TIPS_ENDED);
		}
	}
	...
	destroy()
	{
		...
		{
			this._fTips_arr[i].off(TooltipView.EVENT_ON_TIP_SHOWN, this._onTooltipShown, this);
			this._fTips_arr[i].off(TooltipView.EVENT_ON_TIP_HIDED, this._onTooltipHided, this);
		}
...
export default LobbyTooltipsView;