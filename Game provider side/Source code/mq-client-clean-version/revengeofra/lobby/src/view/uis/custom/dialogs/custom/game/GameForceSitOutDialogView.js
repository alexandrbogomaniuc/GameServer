...
class GameForceSitOutDialogView extends DialogView
{
	...
	_setMessage(messageId)
	{
		...
		if (this.uiInfo.compensateSpecialWeapons > 0 || this.uiInfo.totalReturnedSpecialWeapons > 0)
		{
			...
			let moneyTextLeft = lTextContainer_sprt.addChild(new TextField(GameForceSitOutDialogView.MONEY_TEXT_FORMAT));
			moneyTextLeft.maxWidth = 85;
			moneyTextLeft.anchor.set(1, 0.5);
			moneyTextLeft.position.set(0, 0);

			moneyTextLeft.text = "+ " + "\u200E" + (APP.playerController.info.currencySymbol || "") + "\u200E";

			let moneyTextRight = lTextContainer_sprt.addChild(new TextField(GameForceSitOutDialogView.MONEY_TEXT_FORMAT));
			moneyTextRight.maxWidth = 85;
			moneyTextRight.anchor.set(0, 0.5);
			moneyTextRight.position.set(0, 0);

			let lCompensateAndReturnMoney_num = this.uiInfo.compensateSpecialWeapons + this.uiInfo.totalReturnedSpecialWeapons;
			moneyTextRight.text = NumberValueFormat.formatMoney(lCompensateAndReturnMoney_num);
		}
	}
