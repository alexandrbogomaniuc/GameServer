...
class CommonPanelView extends SimpleUIView
{
	...
	_initCommonPanelView(aPanelStages_st_arr)
	{
		...
		//BALANCE...
		this._fBalanceBlock_bb = lStageView_sprt.addChild(new BalanceBlock(this.uiInfo.gameCurrencySymbol, this.uiInfo.gameIndicatorsUpdateTime));
		this._fBalanceBlock_bb.on(BalanceBlock.EVENT_REFRESH_BALANCE_REQUEST, this.emit, this);
		//...BALANCE

		//WIN...
		this._fWinBlock_wb = lStageView_sprt.addChild(new WinBlock(this.uiInfo.gameCurrencySymbol, this.uiInfo.gameIndicatorsUpdateTime));
	...
	_updateUI()
	{
		...
		this._fBalanceBlock_bb.updateCurrency(this.uiInfo.gameCurrencySymbol);
		...
		if (this.uiInfo.gameUIVisible && !this.uiInfo.isWebGlContextLost && !this._fTournamentModeInfo_tmi.isTournamentCompletedOrFailedState)
		{
			...
			if (this.uiInfo.playerSatIn)
			{
				...
				this._fWinBlock_wb.updateCurrency(this.uiInfo.gameCurrencySymbol);
			}
		}
		else
		{
	...
	_validateButtons()
	{
		...
		{
			this._fFireSettingsButton_btn.visible = true;
	...
	_updateButtonsPositions()
	{
		if (!APP.isMobile)
		{
			...
			this._fFireSettingsButton_btn.position.set(this._btnsPos[5].x, this._btnsPos[5].y);
		}
		else
		{
	...
	{
		this._fButtonsGroupContainer_sprt.visible = false;
		if (APP.isMobile && this.uiInfo.gameUIVisible)
		{
			this._fGroupButton_btn.visible = false;
		}
	}
