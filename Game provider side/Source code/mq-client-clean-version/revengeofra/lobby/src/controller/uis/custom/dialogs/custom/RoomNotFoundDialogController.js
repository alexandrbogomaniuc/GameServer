...
class RoomNotFoundDialogController extends DialogController
{
	...
	__validateViewLevel ()
	{
		...
		if (info.isActive)
		{
			...
			else if (this.info.roomSelectionErrorCode === LobbyWebSocketInteractionController.ERROR_CODES.TOO_MANY_PLAYER)
			{
				messageId = "TADialogMessageTooManyPlayers";
			}
			else if (this.info.roomSelectionErrorCode === LobbyWebSocketInteractionController.ERROR_CODES.TOO_MANY_OBSERVERS)
			{
				messageId = "TADialogMessageTooManyObservers";
			}
