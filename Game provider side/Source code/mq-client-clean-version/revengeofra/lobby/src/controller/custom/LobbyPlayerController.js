...
class LobbyPlayerController extends PlayerController
{
	...
	_onPicksUpSpecialWeaponsChangeStateRequestConfirmed(event)
	{
		let lUpdatedData_obj = {};
	...
	_onServerMessage(event)
	{
		...
		switch(data.class)
		{
			case SERVER_MESSAGES.ENTER_LOBBY_RESPONSE:
				...
				lUpdatedData_obj[PlayerInfo.KEY_MONEY_WHEEL_PAYOUTS] = {value: data.paytable.moneyWheelPayouts};
				...
				if (APP.tournamentModeController.info.isTournamentMode)
				{
					lUpdatedData_obj[PlayerInfo.KEY_REAL_CURRENCY_SYMBOL] = {value: data.currency && data.currency.symbol || "", time: data.date};

					delete data.currency;
				}
				else
				{
					lUpdatedData_obj[PlayerInfo.KEY_CURRENCY_SYMBOL] = {value: data.currency && data.currency.symbol || "", time: data.date};
				}
