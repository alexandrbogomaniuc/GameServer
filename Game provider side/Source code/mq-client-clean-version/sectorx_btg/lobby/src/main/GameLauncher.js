...
class GameLauncher extends EventDispatcher
{
	...
	addGame(gameUrl, aOptAlreadySeatRoomId_num = -1)
	{
		...
		let commonPathForActionGames = APP.appParamsInfo.commonPathForActionGames;

        if(commonPathForActionGames && commonPathForActionGames.length>0){
            gameUrl+= `&commonPathForActionGames=${commonPathForActionGames}`;
        }
