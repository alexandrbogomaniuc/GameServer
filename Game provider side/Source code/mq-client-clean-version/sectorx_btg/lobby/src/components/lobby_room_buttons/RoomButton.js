...
class RoomButtom extends Sprite
{
	...
	static get VISIBLE_OFFSET_X()				{return 167;}
	...
	_getCard(aSkinId_int)
	{
		if(!this._fCards_s_arr[aSkinId_int])
		{
			...
			let l_btn = this._fCardsContainer_s.addChild(new Button("lobby/button_incons/lobby_room_level_" + 1, null, true));
	...
	_mouseOver()
	{
		let lSequenceScale_arr = [
			{tweens: [{ prop: "scale.x", to: 1.05 }, { prop: "scale.y", to: 1.05 }], duration: 1 * FRAME_RATE},
			];
