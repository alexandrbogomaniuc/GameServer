...
class AutofireOnOffButton extends PointerSprite
{
	...
	constructor()
	{
		...
		this._fIsActive_bl = APP.gameScreen.fireController.autofireButtonEnabled;
	...
	_init()
	{
		...
		{
			this._fONButtonView_spr = this._fButtonContainer_spr.addChild(APP.library.getSprite("player_spot/battleground/autofire_button_on"));
			this._fOFFButtonView_spr = this._fButtonContainer_spr.addChild(APP.library.getSprite("player_spot/battleground/autofire_button_off"));
		}else
		{
		...
		this._fText_ta.position.set(-0.5, -1.5);
	...
	_onClick()
	{
		if(APP.gameScreen.gameFieldController.isFinalCountingFireDenied) return;

		this._fButtonContainer_spr.scale.set(1.32);
	}
