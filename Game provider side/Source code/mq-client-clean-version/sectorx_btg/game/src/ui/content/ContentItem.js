...
class ContentItem extends Sprite
{
	...
	constructor(itemInfo)
	{
		...
		this._masterSeatId = APP.playerController.info.seatId;
	...
	_addPrizeView()
	{
		...
		if (assetName !== undefined)
		{
			...
			if (itemInfo.isWeaponType && itemInfo.weaponId == WEAPONS.HIGH_LEVEL)
			{
				...
				for (let i = 0; i < 3; i++)
				{
					let lNormalStripe_spr = this._prizeView.addChild(APP.library.getSprite("battleground/powerup/stripe_normal"));
				...
				let lWeaponView = APP.library.getSpriteFromAtlas(`weapons/DefaultGun/default_turret_${lHighLevelWeaponSkin}/turret_top`);
	...
	{
		return {container: APP.gameScreen.gameFieldController.awardingContainerInfo.keySparklesContainer, zIndex: APP.gameScreen.gameFieldController.awardingContainerInfo.zIndex}
	}
	...
	get _finalLocalPosition()
	{
		...
		let playerSeat = APP.gameScreen.gameFieldController.getSeat(this._itemInfo.playerSeatId);
		if (!playerSeat)
		{
			let playerSpot = APP.currentWindow.gameFieldController.spot;
		...
		if (playerSeat)
		{
			if (this._itemInfo.isWeaponType)
			{
				...
				{
					globalPos = APP.currentWindow.gameFieldController.getWeaponContentLandingPosition(this._itemInfo.weaponId);
				}
				else
				{
