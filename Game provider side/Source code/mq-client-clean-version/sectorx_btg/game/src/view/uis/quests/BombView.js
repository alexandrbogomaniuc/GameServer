...
class BombView extends SimpleUIView
{
	...
	_createExplosion()
	{
		let lExplision = this._fExplosion_sprt = this.addChild(APP.library.getSprite("bomb/explosion"));
	...
	_createCircleBlast()
	{
		...
		for (let i = 0; i < CIRCLE_FLOOR_SETTING.length; i++) {
			let lCircleFloor_sprt = this.addChild(APP.library.getSpriteFromAtlas("bomb/circle_blast_floor"));
	...
	_createBombLight()
	{
		let lBomblight_sprt = this._fBombLight_sprt = this._fBombContainer_sptr.addChild(APP.library.getSprite("bomb/bomb_light"))
