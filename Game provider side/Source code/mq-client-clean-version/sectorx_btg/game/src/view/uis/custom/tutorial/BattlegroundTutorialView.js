...
class BattlegroundTutorialView extends SimpleUIView
{
	...
	_getElements(aData_obj)
	{
		...
		this._fScoreboard_spr = this._fBackground_g.addChild(APP.library.getSprite("tutorial/scoreboard_example"));
		this._fScoreboard_spr.scale.set(1.1);
		this._fScoreboard_spr.position.set(lSWBounds_obj.x + lSWBounds_obj.width/2 + 7, lSWBounds_obj.y + lSWBounds_obj.height/2 + 18);

		let lRealSpotBounds_obj = aData_obj.mainSpot;
		let lIsBottom_bl = aData_obj.isSpotAtBottom;
		let lInitSpotPositionY_num = lIsBottom_bl ? lRealSpotBounds_obj.y + lRealSpotBounds_obj.height - 67 : lRealSpotBounds_obj.y + lRealSpotBounds_obj.height - 50;
		let lInitSpotPositionX_num = lIsBottom_bl ? lRealSpotBounds_obj.x + lRealSpotBounds_obj.width/2 + 10 : lRealSpotBounds_obj.x + lRealSpotBounds_obj.width/2 + 10;

		let lSpotExample = lIsBottom_bl ? "_bottom" : "";
		this._fSpot_spr = this._fBackground_g.addChild(APP.library.getSprite("tutorial/spot_example_battleround" + lSpotExample));
		this._fSpot_spr.position.set(lInitSpotPositionX_num, lInitSpotPositionY_num);
	
		this._fProTip_spr = this._fBackground_g.addChild(APP.library.getSprite("tutorial/pro_tip"));
		this._fProTip_spr.position.set(this._fBGWidth_num/2, lIsBottom_bl ? this._fBGHeight_num/2 - (APP.isMobile ? 160 : 120) : this._fBGHeight_num/2 + (APP.isMobile ? 160 : 130));
	}
	...
	__prepareBatllegroundLines(aData_obj)
	{
		...
		let lPositionOffsets = [
			{ textX: 165, textY: ( APP.isMobile ? -65 : -97 ), x: [25, 60, 195] , y: ( APP.isMobile ? [-30, -64, -64] : [-45, -95, -95] ) },
			{ textX: 42, textY: ( APP.isMobile ? -120 : -123 ), x: [8, 8, lRealSpotBounds_obj.width*1/3 + 8, -lRealSpotBounds_obj.width*1/3, 8] , y: ( APP.isMobile ? [-55, -117, -117, -117, -117,] : [-75, -120, -120, -120, -120] ) },
			{ textX: -80, textY: -103, x: [0,-40,-176] , y: [-35, -100, -100] },
			{ textX: 147, textY: 60, x: [8, 43, 173] , y: [-10, 65, 65] },
			{ textX: -53, textY: 54, x: [6,-26,-154] , y: [-3, 59, 59] },
			{ textX: -69, textY: 37, x: [-8,-38,-174] , y: [-20, 43, 43] }
		]
		...
		//AMMUNITION...
		lPositionOffsets = [
			{ textX: -80, textY: -12, x: [35, -85] , y: [-13, -13] }, //top
			{ textX: -88, textY: 7, x: [30, -90] , y: [8, 8] }, //bottom
			{ textX: 30, textY: 54, x: [25, 25, 105] , y: [-14, 55, 55] }, //top left
			{ textX: 25, textY: APP.isMobile ? -48 : -41, x: [26, 26, 108] , y: ( APP.isMobile ? [10, -50, -50] : [13, -40, -40] ) }, //bottom left
		];
		...
		lHint_obj = {
			...
			textPosition:
			{
				...
				y: lSWBounds_obj.y + lSWBounds_obj.height/2 + lSpotOffsets_obj.textY + lYGapForLeaderBoard_num
			},
			points:
			(lPositionId_num != 3) ?
			[
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + lSpotOffsets_obj.x[0], y: lSWBounds_obj.y + lSWBounds_obj.height/2 + lSpotOffsets_obj.y[0] + lYGapForLeaderBoard_num, dot: true},
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + lSpotOffsets_obj.x[1], y: lSWBounds_obj.y + lSWBounds_obj.height/2 + lSpotOffsets_obj.y[1] + lYGapForLeaderBoard_num, dot: false},
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + lSpotOffsets_obj.x[2], y: lSWBounds_obj.y + lSWBounds_obj.height/2 + lSpotOffsets_obj.y[2] + lYGapForLeaderBoard_num, dot: false},
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + lSpotOffsets_obj.x[3], y: lSWBounds_obj.y + lSWBounds_obj.height/2 + lSpotOffsets_obj.y[3] + lYGapForLeaderBoard_num, dot: false},
			]
			:
			[
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + lSpotOffsets_obj.x[0], y: lSWBounds_obj.y + lSWBounds_obj.height/2 + lSpotOffsets_obj.y[0] + lYGapForLeaderBoard_num, dot: true},
		...
		lHint_obj = {
			...
			textPosition:
			{
				x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 175,
				y: lSWBounds_obj.y + lSWBounds_obj.height - 25
			},
			points: [
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 75, y: lSWBounds_obj.y + lSWBounds_obj.height - 35, dot: true},
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 240, y: lSWBounds_obj.y + lSWBounds_obj.height - 35, dot: false},
			]
		};
		...
		lHint_obj = {
			...
			textPosition:
			{
				x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 135,
				y: lSWBounds_obj.y + lSWBounds_obj.height - 5  + (APP.isMobile ? 10 : 0 )
			},
			points: [
	...
	_tryToHideByClick(e)
	{
		console.log(!Utils.isPointInsidePolygon(this._fShowAgainContainer_spr.globalToLocal(e.data.local), this._fShowAgainContainer_spr.hitArea));
		if (
