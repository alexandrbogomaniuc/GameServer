...
class BulletCapsuleFeatureView extends SimpleUIView
{
	...
	_startSingeSparkAnimation(aRotation_num)
	{
		...
		let lTimeline_mtl = this._fTimeline_arr_mtl[this._fHitAnimationContainer_arr_sprt.length-1];
		...
		lTimeline_mtl.addAnimation(
			...
			0,
			[
				[65 * Math.sin(aRotation_num), 1 * FRAME_RATE],
				[106 * Math.sin(aRotation_num), 1 * FRAME_RATE],
				[138 * Math.sin(aRotation_num), 1 * FRAME_RATE],
				[185 * Math.sin(aRotation_num), 1 * FRAME_RATE],
				[216 * Math.sin(aRotation_num), 1 * FRAME_RATE],
				[458 * Math.sin(aRotation_num), 5 * FRAME_RATE]
			]
		);
		...
		lTimeline_mtl.addAnimation(
			...
			0,
			[
				[-70 * Math.cos(aRotation_num), 1 * FRAME_RATE],
				[-116 * Math.cos(aRotation_num), 1 * FRAME_RATE],
				[-151 * Math.cos(aRotation_num), 1 * FRAME_RATE],
				[-205 * Math.cos(aRotation_num), 1 * FRAME_RATE],
				[-239 * Math.cos(aRotation_num), 1 * FRAME_RATE],
				[-476 * Math.cos(aRotation_num), 5 * FRAME_RATE]
			]
		);

		lTimeline_mtl.callFunctionOnFinish(this._flareAnimationEnd, this, lSpark_sprt);
	}

	_flareAnimationEnd(lSpark_sprt)
	{
