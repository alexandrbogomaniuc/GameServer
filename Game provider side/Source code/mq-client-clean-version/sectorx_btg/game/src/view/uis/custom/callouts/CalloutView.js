...
class CalloutView extends SimpleUIView
{
	...
	constructor()
	{
		...
		this._fBattlegroundXPosition_num = 0;
	...
	_addContainers()
	{
		...
		let lBaseContainerXPosition_num;
		if (APP.isBattlegroundGame)
		{
			lBaseContainerXPosition_num = APP.config.size.width + INDENT_X;
			this._fBattlegroundXPosition_num = lBaseContainerXPosition_num;
		}
		else
		{
			lBaseContainerXPosition_num = -INDENT_X;
		}

		baseContainer.position.set(lBaseContainerXPosition_num, 100);
		...
		messageContainer.position.set(APP.isBattlegroundGame? 30 : 0, 0);
	...
	_addCalloutBase()
	{
		...
		lPanel_spr.scale.x = APP.isBattlegroundGame ? -1 : 1;
	...
	_showCallout()
	{
		...
		this._fBaseContainer_spr.moveXTo(APP.isBattlegroundGame ?  this._fBattlegroundXPosition_num - 250: INDENT_X, 300, Easing.linear.easeIn, this._startLightSweepAnimation.bind(this));
	}
	...
	{
		this._fBaseContainer_spr.removeTweens();
		this._fBaseContainer_spr.moveXTo(APP.isBattlegroundGame ? this._fBattlegroundXPosition_num : -INDENT_X, 300, Easing.linear.easeIn, this._onCalloutAnimationCompleted.bind(this));
	}
	...
	_initLightSweep()
	{
		if (this._profilingInfo.isVfxProfileValueMediumOrGreater && !this._fSweep_sprt)
		{
			...
			let lLightSweepPositionX_num;
			let lMessagePositionX_num;

			if (APP.isBattlegroundGame)
			{
				lMessagePositionX_num = 30;
				lLightSweepPositionX_num = -45; 
			}
			else
			{
				lMessagePositionX_num = 0;
				lLightSweepPositionX_num = -75; 
			}

			this._fMessageContainer_spr.position.set(lMessagePositionX_num, 0);

			let lMask_sprt = new PIXI.Sprite(l_txtr);
			this._fSweep_sprt.mask = lMask_sprt;

			lMask_sprt.position.set(lLightSweepPositionX_num, 0);
