...
class BattlegroundResultScreenView extends SimpleUIView {
	...
	_updatePlayerWonField() {
		...
		if (l_rrsi.winnersCount <= 1) {
			...
			else //refund, l_rrsi.winnersCount === 0
			{
				...
				let lisWithoutRake = ((APP.gameScreen.gameFieldController.roundResultScreenController.info.battlegroundBuyIn !== null) && (l_rrsi.playerTotalPrize * 100 !== APP.gameScreen.gameFieldController.roundResultScreenController.info.battlegroundBuyIn))
	...
	_createSmallPanel() {
		...
		if (l_rrsi.isPlayerWon) {
			...
			this._fWinPanelCountValue_rrtiv.position.set(-220, 10);
