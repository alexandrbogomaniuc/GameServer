...
class MoneyWheelAward extends CoinsAward
{
	...
	_initSprites()
	{
		...
		if (APP.profilingController.info.isVfxProfileValueMediumOrGreater)
		{
			this._fFlare_spr = this._fBackgroundContainer_spr.addChild(APP.library.getSprite("common/misty_flare"));
	...
	_startCaptionLightSweep()
	{
		...
		let lFirstSweep_spr = this._fSweep_sprt.addChild(APP.library.getSprite("money_wheel/you_win_lightsweep"));
		lFirstSweep_spr.rotation = -1.1;
		lFirstSweep_spr.scale.set(4);

		let lSecondSweep_spr = this._fSweep_sprt.addChild(APP.library.getSprite("money_wheel/you_win_lightsweep"));
