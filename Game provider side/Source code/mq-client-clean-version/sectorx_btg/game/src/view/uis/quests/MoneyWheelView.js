...
import { MONEY_WHEEL_ENABLED } from '../../../../../shared/src/CommonConstants';
...
class MoneyWheelView extends SimpleUIView
{
	...
	_updateValuePayout(aBetLevel_num)
	{
		...
		for (let lPayoutBase_num in PAYOUTS_PART_ID) {
			let lWinPayout_num = lPayoutBase_num * (APP.isBattlegroundGame ? FIXED_BATTLEGOUND_STAKE : APP.playerController.info.currentStake) * aBetLevel_num;
	...
	_startAnimation(aValuePayout_num, aBetLevel_num)
	{
		...
		if(MONEY_WHEEL_ENABLED)
		{
			...
			let lWinPayoutBase_num = aValuePayout_num / ((APP.isBattlegroundGame ? FIXED_BATTLEGOUND_STAKE : APP.playerController.info.currentStake) * aBetLevel_num);
	...
	{
		console.log("MoneyWheel problem show");
		...
		let lTargetScale_num = (APP.isMobile ? 0.42 : 0.47)
		...
		let lScaleFirst_seq = [
			...
			{
				tweens: [{prop: "scale.x", to: lTargetScale_num}, {prop: "scale.y", to: lTargetScale_num}],
				duration: 12*this._fLocalFrameRate_num
			},
		];
	...
	_sequenceAfterFirstScale()
	{
		...
		let lFinalPositionX_num = lGlobalPosition_pt.x + (APP.isMobile ? 170 : 330) - (this._fIsBossHpBarExist_bln && !APP.isMobile ? 70 : 0);
		let lFinalPositionY_num = lGlobalPosition_pt.y + (APP.isMobile ? 65 : -15);
	...
	_resetView()
	{
		console.log("MoneyWheel problem hide ");
