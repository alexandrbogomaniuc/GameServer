...
class GamePleaseWaitDialogController extends SimpleController
{
	...
	__initControlLevel()
	{
		...
		this._fRoundResultScreenController_rrsc.on(RoundResultScreenController.EVENT_ON_FAILED_TO_SHOW_ROUND_RESULT_SCREEN_DUE_TO_WEAPONS_NO_FIRED_IS_ACTIVE, this._onRoundResultShowFailed, this);
	}
	...
	_onClientSentMessage(aData_obj)
	{
		switch (aData_obj.class)
		{
			...
			case CLIENT_MESSAGES.CONFIRM_BATTLEGROUND_BUY_IN:
				this._sendOpenDialogExternalMessage(`${WAIT_MESSAGE_TYPES.CONFIRM_BATTLEGROUND_BUY_IN}:${aData_obj.rid}`, 2000);
				break;
	...
	_onServerMessage(aEvent_obj)
	{
		...
		switch (lServerMessageData_obj.class)
		{
			...
			case SERVER_MESSAGES.OK:
				...
				else if (APP.isBattlegroundGame && lRequestData_obj && lRequestData_obj.class == CLIENT_MESSAGES.CONFIRM_BATTLEGROUND_BUY_IN)
				{
					this._sendCloseDialogExternalMessage(`${WAIT_MESSAGE_TYPES.CONFIRM_BATTLEGROUND_BUY_IN}:${lRequestData_obj.rid}`);
				}
				break;
