...
class TargetingController extends SimpleUIController {
	...
	tick()
	{
		if (this.info.isTargetEnemyDefined || APP.gameScreen.fireController.autofireButtonEnabled &&  APP.isAutoFireMode)
		{
			if(APP.gameScreen.fireController.autofireButtonEnabled && !this.info.isTargetEnemyDefined && APP.isAutoFireMode)
			{
	...
	_onTargetingB3Invulnerable(event)
	{
		if (this.info.isActive && this.info.targetEnemyId === event.enemyId)
		{
			...
			this.info.rememberedUserSelectedStateUntilInvulnerable = this.isUserSelectedEnemy;
	...
	_onTargetingB3Vulnerable()
	{
		if (this.info.isActive && this.info.rememberedTargetUntilInvulnerable)
		{
			...
			this._fIsUserSelectedEnemy_bl = this.info.rememberedUserSelectedStateUntilInvulnerable;
			
			this.info.rememberedTargetUntilInvulnerable = null;
			this.info.rememberedUserSelectedStateUntilInvulnerable = false;
		}
	}
	...
	_tryToSelectNextTarget()
	{
		...
		{
			if (!APP.gameScreen.fireController.autofireButtonEnabled)
			{
				this._resetNextTargetTimer();
				this._resetTarget();
				this._resetB3FormationTarget();
	...
	_getNextMostValuableEnemyId()
	{
		...
		if (APP.isBattlegroundGame)
		{
			...
			{
				let lGunPos_p = this._fFireController_fc.getGunPosition(this._fFireController_fc.seatId);
				if (!lGunPos_p)
				{
					return null;
				}
