...
public abstract class AbstractStartGameUrlHandler<MESSAGE extends TInboundObject, CLIENT extends ILobbySocketClient>
    ...
    public static String getRoomUrl(WebSocketSession session, long roomId, IServerConfig serverConfig,
        ...
        if (host.equals("localhost") || host.endsWith("mp.local") || host.endsWith("mp.local.com")
                || host.endsWith(".mydomain")) { // hack for local/dev deploy
            // Fix: Use 8080 for localhost (MP server), 8081 might be for GS
            String port = "8080";
            roomWebSocketUrl = IMessageHandler.getWsProtocol(origin) + host + ":" + port + "/websocket/";
        } else {
