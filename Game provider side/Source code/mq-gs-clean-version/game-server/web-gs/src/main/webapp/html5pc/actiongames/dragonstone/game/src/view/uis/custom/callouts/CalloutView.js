...
class CalloutView extends SimpleUIView
{
	...
	constructor()
	{
		...
		this.fStartSweepX_num = -70;
		this.fEndSweepX_num = 140;
	...
	setSweepBounds(leftBound = -70, rightBound = 140)
	{
	...
	_addContainers()
	{
		...
		baseContainer.position.set(APP.config.size.width + INDENT_X, 161);
	...
	_addFirstLineCaption(firstCaptionId)
	{
		...
		lCaption_spr.scale.set(0.8);
		lCaption_spr.position.set(this.fMaskX, 50 - 16);
	...
	_addSecondLineCaption(secondCaptionId)
	{
		...
		lCaption_spr.position.set(this.fMaskX, 50 - 2);
		lCaption_spr.scale.set(this.fCaptionScale);
		messageContainer.addChild(lCaption_spr);
	}
	...
	_showCallout()
	{
		...
		let l_seq = [{tweens: [{ prop: "position.x", to: APP.config.size.width - INDENT_X }],
			duration: 300,
	...
	_initLightSweep()
	{
		...
		{
			this._fSweep_sprt = this._FXContainer.addChild(APP.library.getSprite("critical_hit/light_sweep"));
			this._fSweep_sprt.scale.set(0.4);
			...
			APP.stage.renderer.render(this._messageContainer, { renderTexture: l_txtr });					// renders just right part of _messageContainer, idk how to fix that
