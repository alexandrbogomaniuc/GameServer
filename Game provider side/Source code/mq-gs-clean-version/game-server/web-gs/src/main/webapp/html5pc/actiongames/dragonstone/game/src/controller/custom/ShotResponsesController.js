...
import { WEAPONS, ENEMIES_EFFECTS_LIST } from '../../../../shared/src/CommonConstants';
...
class ShotResponsesController extends SimpleController {
	...
	_onServerHitMessage(event) {
		...
		if (lShotResponseInfo_sri.lastResult)
		{
	...
	_onServerLastResultResponse()
	{
		...
		for (let enemyId in lMissGroup_obj)
		{
			...
			//rage-damaged wins should be animated separately...
			for (let i=0; i<lShotResponses_sri_arr.length; i++)
			{
				let lShotResponse_sri = lShotResponses_sri_arr[i];
				let lIsRageResponse = lShotResponse_sri.effects ? Boolean(~lShotResponse_sri.effects.indexOf(ENEMIES_EFFECTS_LIST.RAGE)) : false;
				if (lIsRageResponse)
				{
		...
		for (let enemyId in lHitGroup_obj)
		{
			...
			//wins with slot should be animated separatedly...
			for (let i=0; i<lShotResponses_sri_arr.length; i++)
			{
				let lShotResponse_sri = lShotResponses_sri_arr[i];
				if (lShotResponse_sri.slot)
				{
					lShotResponsesQueue_gsri_arr.push(lShotResponse_sri);
					lShotResponses_sri_arr.splice(i, 1);
					i--;
	...
	_groupResponses(aBaseResponse_sri, aAddResponse_sri)
	{
		...
		//add fragment
		if (lAddResponse_sri.fragmentId && lAddResponse_sri.fragmentId > 0)
		{
			lBaseResponse_sri.fragments = lBaseResponse_sri.fragments || [];
			if (!lBaseResponse_sri.fragments.length && lBaseResponse_sri.fragmentId > 0)
			{
				lBaseResponse_sri.fragments.push(lBaseResponse_sri.fragmentId);
			}
			lBaseResponse_sri.fragments.push(lAddResponse_sri.fragmentId);
		}
