...
class GroundFireTraceAnimation extends Sprite
{
	...
	_startGroundBurns()
	{
		let burnsNumber = Math.ceil(2*GROUND_BURNS_CONFIG.length/3);
		let burnCount = 0;
		...
		for (let config of GROUND_BURNS_CONFIG)
		{
			if(!APP.profilingController.info.isVfxProfileValueMediumOrGreater)
			{
				if(burnCount >= burnsNumber) return;
				burnCount ++;
			}
	...
	{
		let groundburn = this.addChild(APP.library.getSprite("common/groundburn"));
	...
	_startSmokes()
	{
		let smokesNumber = Math.ceil(SMOKES_CONFIG.length/3);
		let smokeCount = 0;
		...
		for (let config of SMOKES_CONFIG)
		{
			if(!APP.profilingController.info.isVfxProfileValueMediumOrGreater)
			{
				if(smokeCount >= smokesNumber) return;
				smokeCount ++;
			}
