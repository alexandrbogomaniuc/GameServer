...
class BattlegroundCountDownDialogView extends GameplayDialogView
{
	...
	_addDialogBase()
	{
		this._fBackContainer_sprt = this.addChild(APP.library.getSpriteFromAtlas("gameplay_dialogs/battleground/count_down/count_down_frame_dialog"));
		this._fBackContainer_sprt.scale.set(1.5);
		this._baseContainer.addChild(this._fBackContainer_sprt);
		if(APP.isCAFMode)
		{
			this._fBackContainer_sprt.y = -15;
		}
		//LOGO...
		let logo = I18.generateNewCTranslatableAsset("TABattlegroundCountDownLogo");
		logo.scale.set(1.5);
		logo.position.set(-340, -230);
