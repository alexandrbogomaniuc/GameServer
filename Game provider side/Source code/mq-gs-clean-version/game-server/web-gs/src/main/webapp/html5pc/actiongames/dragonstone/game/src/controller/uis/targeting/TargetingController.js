...
class TargetingController extends SimpleUIController {
	...
	__init()
	{
		...
		this._gameScreen.on(GameScreen.EVENT_ON_NEW_BOSS_CREATED, this._onBossEnemyRising, this);
		this._gameScreen.on(GameScreen.EVENT_ON_BOSS_ENEMY_APPEARANCE_TIME, this._onBossAppearanceTime, this);
	...
	_onBossEnemyRising(event)
	{
		let lIsActive_bln = this.info.isActive || APP.gameScreen.gameField.autofireButtonEnabled;

		this._resetTarget();
		this._fCurTargetPreference_int = undefined;

		this._fIsBossTargetingRequired_bl = lIsActive_bln;

		if(event.isLasthandBossView && this._fIsBossTargetingRequired_bl)
		{
			this._fIsBossTargetingRequired_bl = false;
			this._tryToSelectNextTarget();
		}
	...
	_onBossAppearanceTime()
	{
		if (this._fIsBossTargetingRequired_bl)
		{
			this._fIsBossTargetingRequired_bl = false;
	...
	_getNextMostValuableEnemyId()
	{
		...
		lEnemies_obj_arr = lEnemies_obj_arr.filter(function(value) {
			...
			if (lGameScreen_gs.gameStateController.info.isBossSubround && value.typeId !== ENEMY_TYPES.BOSS)
			{
				return false;
			}
	...
	destroy()
	{
		...
		this._fIsBossTargetingRequired_bl = undefined;
