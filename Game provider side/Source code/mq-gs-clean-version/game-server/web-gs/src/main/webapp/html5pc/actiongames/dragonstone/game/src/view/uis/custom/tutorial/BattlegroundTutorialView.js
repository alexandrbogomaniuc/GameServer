...
import { DropShadowFilter, GlowFilter } from '../../../../../../../common/PIXI/src/dgphoenix/unified/view/base/display/Filters';
...
class BattlegroundTutorialView extends SimpleUIView
{
	...
	_getElements(aData_obj)
	{
		...
		this._fScoreboard_spr.position.set(lSWBounds_obj.x + lSWBounds_obj.width/2 + 7, lSWBounds_obj.y + lSWBounds_obj.height/2 + 44);

		let lDSPanelBounds_obj = {
			height: 307,
			width: 123,
			x: 861.5,
			y: 142.8
		...
		this._fFragmentsPanel_spr = this._fBackground_g.addChild(APP.library.getSpriteFromAtlas("tutorial/dragonstone_example"));
		this._fFragmentsPanel_spr.scale.set(1.6);
		this._fFragmentsPanel_spr.position.set(lDSPanelBounds_obj.x + lDSPanelBounds_obj.width/2 - 9, lDSPanelBounds_obj.y + lDSPanelBounds_obj.height/2/*+ 18*/);
		...
		this._fProTip_spr = this._fBackground_g.addChild(APP.library.getSpriteFromAtlas("tutorial/pro_tip"));
		const lProTip_PositionY_fl = lIsBottom_bl ? this._fBGHeight_num/2 - (APP.isMobile ? 150 : 120) : this._fBGHeight_num/2 + (APP.isMobile ? 170 : 135);
		console.log("Bottom element " + lIsBottom_bl + " and the lPro " + lProTip_PositionY_fl)

		//this._fProTip_spr.position.set(this._fBGWidth_num/2, lIsBottom_bl ? this._fBGHeight_num/2 - (APP.isMobile ? 160 : 90) : this._fBGHeight_num/2 + (APP.isMobile ? 160 : 105));
		this._fProTip_spr.position.set(this._fBGWidth_num/2,lProTip_PositionY_fl);
	}
	...
	__prepareBatllegroundLines(aData_obj)
	{
		...
		lHint_obj = {
			...
			textPosition:
			{
				x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 130 + 30,
				y: lSWBounds_obj.y + lSWBounds_obj.height - 35 + 30
			},
			points: [
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 55, y: lSWBounds_obj.y + lSWBounds_obj.height - 45 + 30, dot: true},
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 200, y: lSWBounds_obj.y + lSWBounds_obj.height - 45 + 30, dot: false},
			]
		};
		...
		lHint_obj = {
			...
			textPosition:
			{
				x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 82 + 30,
				y: lSWBounds_obj.y + lSWBounds_obj.height - 15 + (APP.isMobile ? 10 : 0) + 30
			},
			points: [
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 55, y: lSWBounds_obj.y + lSWBounds_obj.height - 45 + 30, dot: false},
				{ x: lSWBounds_obj.x + lSWBounds_obj.width/2 + 55, y: lSWBounds_obj.y + lSWBounds_obj.height - 45 + 30, dot: false},
			]
		};
		...
		let lDSPanelStartPosition_obj = {
			x: lDSPanelBounds_obj.x + lDSPanelBounds_obj.width/2 - 10,
			y: lDSPanelBounds_obj.y + lDSPanelBounds_obj.height/2 + 95 + (APP.isMobile ? -2 : 0 )
		};

		//SUMMON DRAGON...
		lHint_obj = {
			textAssetName: 'TATutorialSummonDragon',
			textPosition:
			{
				x: lDSPanelStartPosition_obj.x - 45,
				y: lDSPanelStartPosition_obj.y - 50
			},
			points: [
				{ x: lDSPanelStartPosition_obj.x, y: lDSPanelStartPosition_obj.y - 60, dot: true},
				{ x: lDSPanelStartPosition_obj.x - 200, y: lDSPanelStartPosition_obj.y - 60, dot: false}
			]
		};
	...
	_clearElements()
	{
		...
		this._fFragmentsPanel_spr && this._fFragmentsPanel_spr.destroy();
